...
# --- Tick 340/344: open_git_pr never-omit-branch + secrets-first title reminder -----
...
if b:
    print(f"=== open_git_pr anti-churn (Tick 340/344) ===")
...
" 2>/dev/null || true)"
  if [[ -n "${_og_branch}" ]]; then
    echo "=== open_git_pr anti-churn (Tick 340/344) ==="
