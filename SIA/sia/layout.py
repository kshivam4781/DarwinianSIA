...
class Names:
    ...
    AGENT_DNA = "agent_dna.json"
    CIVILIZATION_JSON = "civilization.json"
...
def venv_python_path(venv_dir: str) -> str:
    """Path to the python executable inside a venv (Windows and Unix)."""
...
def venv_pip_path(venv_dir: str) -> str:
    """Path to the pip executable inside a venv (Windows and Unix)."""
    if sys.platform == "win32":
...
class RunLayout:
    ...
    def gen_agent_dir(self, gen: int, agent_id: int) -> str:
        """Path to an individual agent within a darwinian generation."""
        return os.path.abspath(f"{self.run_dir}/gen_{gen}/agent_{agent_id}")

    def agent_dna(self, gen: int, agent_id: int) -> str:
        return os.path.join(self.gen_agent_dir(gen, agent_id), Names.AGENT_DNA)

    @property
    def civilization_json(self) -> str:
        return os.path.join(self.run_dir, Names.CIVILIZATION_JSON)
