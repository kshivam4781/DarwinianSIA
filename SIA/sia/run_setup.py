...
def load_task_files(
    ...
    sample_task_descriptions = Path(paths.sample_descriptions).read_text(encoding="utf-8")
    logger.info("  ✓ Sample task descriptions loaded")

    if resolved_ref is None:
        reference_target_agent_py = Path(paths.reference_agent).read_text(encoding="utf-8")
    else:
        reference_target_agent_py = resolved_ref.inline_seed or ""
    logger.info("  ✓ Reference target agent loaded")

    with open(paths.sample_execution, encoding="utf-8") as f:
        sample_agent_execution = json.load(f)
    logger.info("  ✓ Sample agent execution loaded")

    task_md = Path(paths.task_md).read_text(encoding="utf-8")
...
def setup_run_directory(
    ...
    if os.path.exists(run_directory):
        if not resume:
            ...
            logger.error("Please use a different run_id, pass --resume, or remove the existing directory")
            sys.exit(1)
        logger.info(f"Resume: reusing existing run directory: {run_directory}")
    else:
        logger.info(f"Creating run directory: {run_directory}")
        os.makedirs(run_directory, exist_ok=False)

    if not os.path.exists(meta_agent_working_directory):
        logger.info(f"Creating meta_agent working directory: {meta_agent_working_directory}")
        os.makedirs(meta_agent_working_directory, exist_ok=True)

    venv_dir = layout.venv_dir
    if os.path.isdir(venv_dir) and resume:
        logger.info(f"Resume: reusing virtual environment at: {venv_dir}")
    else:
    ...
    context_path = os.path.join(run_directory, "context.md")
    if resume and os.path.isfile(context_path):
        logger.info("  ✓ Resume: keeping existing context.md")
    else:
