...
def run_evaluation(
    ...
    if eval_subset is not None and eval_subset > 0:
        from sia.eval_subset import evaluate_gen_dir_subset, resolve_task_root

        root = task_root or resolve_task_root(task_dir)
        logger.info(f"  → Eval subset mode: first {eval_subset} samples")
        try:
            results = evaluate_gen_dir_subset(gen_directory, root, eval_subset)
            if results is None:
                logger.warning("  ⚠ Subset evaluation not supported for this task; falling back to full evaluate.py")
            else:
                eval_log_file = os.path.join(gen_directory, Names.EVAL_LOG)
                write_text(eval_log_file, json.dumps(results, indent=2))
                logger.info("  ✓ Subset evaluation completed")
                logger.info(f"    Results: {json.dumps(results, indent=2)}")
                return {
                    "status": "success",
                    "log_path": eval_log_file,
                    "results_path": os.path.join(gen_directory, Names.RESULTS_JSON),
                    "output": json.dumps(results, indent=2),
                    "eval_subset": eval_subset,
                }
        except (OSError, ValueError, KeyError) as e:
            logger.error(f"  ✗ Subset evaluation failed: {e}")
...
def run_generation(
    ...
    results_json_path = os.path.join(gen_dir, Names.RESULTS_JSON)
    skipped_run = resume and os.path.isfile(results_json_path)

    if skipped_run:
        logger.info(f"  → Resume: skipping target agent (existing {Names.RESULTS_JSON})")
        ...
        logger.info("  → Dry-run: running mock target agent subprocess")
    ...
    if skipped_run:
        logger.info("  → Resume: keeping existing evaluation results")
    else:
        run_evaluation(
            gen_dir,
            dataset_dir,
    ...
    if current_gen < max_gen:
        if dry_run:
            ...
            src = layout.target_agent(current_gen)
            dst = layout.target_agent(next_gen)
            if os.path.isfile(src):
                _shutil.copy2(src, dst)
            logger.info(f"Dry-run: copied target agent to gen_{next_gen} (skipped feedback LLM)")
            return
...
def main():
    ...
    from sia.env_loader import load_project_dotenv
    ...
    logger.info(
        f"  - Darwinian mode: {getattr(args, 'darwinian', False)}"
    )
    if getattr(args, "darwinian", False):
    ...
    if getattr(args, "dry_run", False):
        logger.info("  - Dry-run: enabled (no LLM API calls)")
    if getattr(args, "eval_subset", None):
        logger.info(f"  - Eval subset: first {args.eval_subset} samples")
    if getattr(args, "resume", False):
        logger.info("  - Resume: enabled")

    dry_run = getattr(args, "dry_run", False)
    resume = getattr(args, "resume", False)
    eval_subset = getattr(args, "eval_subset", None)
    task_name = args.task or Path(task_dir).name

    if dry_run and not getattr(args, "darwinian", False):
        logger.info("  → Dry-run applies to target execution; meta-agent still runs unless darwinian")
    if dry_run and args.darwinian:
        logger.info("  → Darwinian dry-run skips all meta/feedback LLM calls")
    ...
    if eval_subset is not None and eval_subset > 0:
        ...
        abs_dataset_directory = materialize_subset_dataset(
            task_dir,
        ...
        logger.info(f"  ✓ Eval subset dataset: {abs_dataset_directory}")
    ...
    # ========================
    # SECTION 4: Run Target Agent Creation (Meta-Agent) OR Darwinian Population
    # ========================

    if args.darwinian:
        if args.focus == "weights":
            raise SystemExit("Darwinian mode currently supports harness focus only (not weights mode).")
        if args.elite_count > args.population_size:
            raise SystemExit(
                f"--elite_count ({args.elite_count}) cannot exceed --population_size ({args.population_size})."
            )
        ...
        logger.info("=" * 80)
        logger.info(f"Darwinian evolution completed all {max_gen} generations!")
        logger.info(f"Results saved in: {run_setup.run_directory}")
        logger.info(f"Civilization memory: {RunLayout(run_setup.run_directory).civilization_json}")
        logger.info(f"Context summary: {os.path.join(run_setup.run_directory, Names.CONTEXT_MD)}")
        logger.info("=" * 80)
        return

    # Standard single-lineage mode: meta-agent creates gen_1
    ...
    if dry_run:
        from sia.evolution.dry_run import write_mock_target_agent

        write_mock_target_agent(run_setup.meta_agent_working_directory, task_name)
        logger.info("  ✓ Dry-run: wrote mock target agent for gen_1 (skipped meta-agent LLM)")
    else:
        asyncio.run(
            run_agent(
