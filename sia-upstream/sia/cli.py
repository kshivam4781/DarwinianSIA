...
def _add_run_args(parser: argparse.ArgumentParser, env_config: Config) -> None:
    ...
    # Darwinian AI Civilization — population-based evolution
    parser.add_argument(
        "--darwinian",
        action="store_true",
        help="Enable Darwinian population-based self-improvement (multiple agents compete per generation).",
    )
    parser.add_argument(
        "--population_size",
        type=int,
        default=env_config.DEFAULT_POPULATION_SIZE,
        help=f"Number of agents per generation in darwinian mode (default: {env_config.DEFAULT_POPULATION_SIZE}).",
    )
    parser.add_argument(
        "--elite_count",
        type=int,
        default=env_config.DEFAULT_ELITE_COUNT,
        help=f"Number of top agents that survive each generation (default: {env_config.DEFAULT_ELITE_COUNT}).",
    )
    parser.add_argument(
        "--mutation_rate",
        type=float,
        default=env_config.DEFAULT_MUTATION_RATE,
        help=f"Probability of mutating each DNA trait during breeding (default: {env_config.DEFAULT_MUTATION_RATE}).",
    )
    parser.add_argument(
        "--seed",
        type=int,
        default=None,
        help="Random seed for reproducible DNA initialization and breeding (darwinian mode).",
    )
    parser.add_argument(
        "--dry-run",
        dest="dry_run",
        action="store_true",
        help="Mock meta/feedback/target agents — no LLM API calls; tests evolution loop for $0.",
    )
    parser.add_argument(
        "--eval_subset",
        type=int,
        default=None,
        metavar="N",
        help="Use only the first N benchmark samples (dataset + evaluation). For cheap dev runs.",
    )
    parser.add_argument(
        "--resume",
        action="store_true",
        help="Skip agents/generations that already have results.json / target_agent.py.",
    )
    parser.add_argument(
        "--baseline_seed",
        nargs="?",
        const="runs/run_201/gen_1/target_agent.py",
        default=None,
        metavar="PATH",
        help="Darwinian gen 1 only: copy a proven target_agent.py instead of meta-agent creation "
        "(default: runs/run_201/gen_1/target_agent.py). Gen 2+ still uses feedback breeding.",
    )
    parser.add_argument(
        "--cabs",
        action="store_true",
    ...
    parser.add_argument(
        ...
        default=None,
        metavar="PATH",
        help="Override path to CABS belief_store (default: {run_dir}/belief_store).",
    )
