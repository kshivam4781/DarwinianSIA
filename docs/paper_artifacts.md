...
**Status:** offline mechanism pack + synthetic B vs D pilot (Tick 23; post-steering H2 case study) + GPQA CLI harness dry-run `run_1800` (Tick 21) + live G2/G3/G4 runners + paper pack + unified pipeline (Ticks 24–29) + Cursor env drafts + **Tick 265 Astral uv bootstrap** (`ensure_uv_on_path`) + **Tick 266 runtime-deps bootstrap** (`ensure_icml_runtime_deps`) + **Tick 267 secrets-only live gate verified** on SYSTEM boot + **Tick 268 secrets-first** + **Tick 269 tip lineage recover/refuse** (`icml_recover_tip.py` / `icml_tip_status.json`) + **Tick 270 main-boot bash tip recover** (`icml_boot_recover.sh` + AGENTS.md). **Tick 275 G2/G3/G4 fetch_diamond_ok gate**. **Tick 277 `.env` + local diamond CSV unlock**. **Tick 278 runner CSV autowire** (`autowire_diamond_csv`). **Tick 279 uv-first runtime package bootstrap** (`uv pip` before `pip --user`). **Tick 280 uv pip `--target` user site** (writable `~/.local` site-packages; avoids Permission denied on `/usr/local`). **Tick 281 user-site on PYTHONPATH**. **Tick 282 deps-before-diamond-fetch** (`ensure_deps_before_diamond_fetch` before HF materialize). **Tick 283 live budget reconcile** (`bump_spent_reconciled` from actual `total_cost_usd`). **Tick 286 ephemeral-dirt tip recover** + zero `docs/icml_budget_spent.json`. **Tick 287 host pandas-free GPQA eval_subset**. **Tick 288 Nebius target profile** (`kimi-nebius-target`) + GPQA reference retarget (Tinker→Nebius/Kimi). **Tick 289 Nebius pydantic-ai meta** (`kimi-nebius-pydantic-meta`) — Anthropic optional for live secrets. No publishable **live** GPQA figures/tables yet (blocked on NEBIUS + HF/CSV).
