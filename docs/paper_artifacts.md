...
**Status:** offline mechanism pack + synthetic B vs D pilot (**Tick 397–405** post-adoption H2 at live Nebius shape pop4×eval5×max_gen6; IDs `1930–1934` / `1940–1944`; Tick-396 `1910–1924` superseded) + GPQA CLI harness dry-run `run_1800` (Tick 21) + live G2/G3/G4 runners + paper pack + unified pipeline (Ticks 24–29) + Cursor env drafts + **Tick 265–405** live stack hardening (Tick 405: dry-run FEEDBACK_PROMPT proves delay-all scoped-feedback skip; `run_1952`). No publishable **live** GPQA figures/tables yet (blocked on NEBIUS + HF/CSV; tip still unmerged to `main`).
