...
**Status:** offline mechanism pack + synthetic B vs D pilot (**Tick 397–403** post-adoption H2 at live Nebius shape pop4×eval5×max_gen6; IDs `1930–1934` / `1940–1944`; Tick-396 `1910–1924` superseded) + GPQA CLI harness dry-run `run_1800` (Tick 21) + live G2/G3/G4 runners + paper pack + unified pipeline (Ticks 24–29) + Cursor env drafts + **Tick 265–403** live stack hardening (Tick 403: delay-all also gates committee `technique_seeds` inject). No publishable **live** GPQA figures/tables yet (blocked on NEBIUS + HF/CSV; tip still unmerged to `main`).
...
- Tick **397** post-adoption H2 tail (last 2 gens, floor gen≥3) recovers seed 22 preferred 0.44→**0.75** so offline MECHANISM is **5/5**; Tick **398** surfaces that same window in the case study (`run_1940` post-adoption share **0.875**). Tick **399** syncs judge-facing SUBMISSION/PRESENTATION/present demo to `1930–1944` and restores summary Fig paths. Tick **400** syncs operator-facing `ICML_HUMAN_UNBLOCK.md` dual-unblock to `1930–1934` / `1940–1944`. Tick **401** syncs root `README.md` evidence checklist to the same IDs. Tick **402** makes delay-all breed logs mark CABS bias `(deferred until gen≥2…)` vs `(applied)` so gen1→gen2 fair mutate is not misread as steered (`run_1951`). Tick **403** gates committee `technique_seeds` inject on the same delay-all flag so deferred logs match DNA (no seed leak on fair early breed). Still **not** live GPQA — publishable PRIMARY requires Nebius multi-seed evidence.
