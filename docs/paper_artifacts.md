...
**Status:** offline mechanism pack + synthetic B vs D pilot (Tick 23; post-steering H2 case study) + GPQA CLI harness dry-run `run_1800` (Tick 21) + live G2/G3/G4 runners + paper pack + unified pipeline (Ticks 24–29) + Cursor env drafts + **Tick 265 Astral uv bootstrap** (`ensure_uv_on_path`) + **Tick 266 runtime-deps bootstrap** (`ensure_icml_runtime_deps`; `--fetch-diamond` no longer Portal-Save-gated) + Portal Save target env `31d13f14-…` / build `5a2d7f34`. No publishable **live** GPQA figures/tables yet (blocked on API secrets).
