...
**Status:** offline mechanism pack + synthetic B vs D pilot (**Tick 397–398** post-adoption H2 at live Nebius shape pop4×eval5×max_gen6; IDs `1930–1934` / `1940–1944`; Tick-396 `1910–1924` superseded) + GPQA CLI harness dry-run `run_1800` (Tick 21) + live G2/G3/G4 runners + paper pack + unified pipeline (Ticks 24–29) + Cursor env drafts + **Tick 265–398** live stack hardening. No publishable **live** GPQA figures/tables yet (blocked on NEBIUS + HF/CSV; tip still unmerged to `main`).
...
- Tick **397** post-adoption H2 tail (last 2 gens, floor gen≥3) recovers seed 22 preferred 0.44→**0.75** so offline MECHANISM is **5/5**; Tick **398** surfaces that same window in the case study (`run_1940` post-adoption share **0.875**). Still **not** live GPQA — publishable PRIMARY requires Nebius multi-seed evidence.
