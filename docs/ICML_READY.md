...
- Evidence: offline synthetic pilot `1750–1754` vs `1760–1764` (Tick 19; Tick 17 ε-greedy path + Tick 18/19 H5 protocol) — D gens30 wins **3/5** (B: 0), D final wins **5/5**, mean final gap ~**5.35pp**. Offline PRIMARY-shaped signal only — **not** live GPQA; leave unchecked for READY. Live → `docs/paper_artifacts.md` Table 1
...
- [x] Documented case study (tie → contradiction → different DNA → fitness lift) with artifacts — offline dry-run `docs/case_study_offline.md` + `run_1763` (`stepwise` preferred → gen2 share **0.25**; lift +0.0869)
...
- Evidence: offline multi-seed Condition D `1760–1764` (`max_gen=6`) → ρ>0.3 on **5/5** seeds (0.8 / 0.8 / 0.8 / 1.0 / 0.6) using population-mean forward Δfitness (`delta_horizon=2`) and gen≥2 pairs (delay-all steered window + ε-lag). Still **not** live GPQA.
...
| Gate | Status |
|------|--------|
| G0 mechanism unit tests | **PASS** (2026-08-03; + delay-all Tick 14; + compressed fitness Tick 16; + ε-greedy/live harvest Tick 17; + H5 protocol Tick 18–19) |
| G1 dry-run Condition D | **PASS** (2026-08-04) — `run_1401` + `test_cabs_inline_dry_run.py` |
| G2 smoke GPQA subset | BLOCKED (no API keys; secrets requested 2026-08-05) |
| G3 pilot B vs D | Offline synthetic pilot refreshed (Tick 19; gens30 **3/5**; H5 **5/5**); **live** G3 NOT STARTED |
| G4 5-seed + metrics | NOT STARTED (live) |
| G5 paper pack | PARTIAL (offline figs + case study + offline PRIMARY gens30 + offline H5 5/5); live pack NOT STARTED |
