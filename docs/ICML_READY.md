...
_Tick 438: on Tick 437 rebase, auto-merge durable/paper-pack-only conflicts (union ``budget_spent`` spend/stages; merge ``prior_live`` gates; demote READY) + ``rebase --continue`` — closes concurrent tip VMs both writing spend → abort → local-only. Tick 437 NF rebase+retry + Tick 436 fetch-before-FF + Tick 435 FF-when-already-on-tip-behind remain. Tip PR #337 remains **MERGEABLE/CLEAN**. Live PRIMARY still blocked on NEBIUS + HF/CSV._
