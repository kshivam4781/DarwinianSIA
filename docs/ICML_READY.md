...
_Tick 430: durable-stash redundancy compares to committed `git show HEAD:<path>` (closes Tick 429 WT-compare wipe after reinject + commit-noop). Tick 429 redundant-only consume + Tick 428 consume-after-push + Tick 427 park/reinject remain. Tip PR #337 remains **MERGEABLE/CLEAN**. Live PRIMARY still blocked on NEBIUS + HF/CSV._
