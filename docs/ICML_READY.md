...
_Tick 434: when HEAD is **not** a tip descendant (greenfield boot behind `origin/<tip>`), checkout tip (preserving durable/companion dirt) instead of skipping — pre-434 committed on boot then non-FF rejected tip push (spend only on invisible boot). Tick 433 tip←HEAD sync when descendant + Tick 432 always→tip PR head remain. Tip PR #337 remains **MERGEABLE/CLEAN**. Live PRIMARY still blocked on NEBIUS + HF/CSV._
