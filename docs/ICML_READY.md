...
- Cursor env: `.cursor/environment.json` (+ **uv**) + Tick 266 env `31d13f14-…` (build `5a2d7f34` SUCCEEDED + proposed) + **Astral uv + runtime-deps bootstrap in G2/G3/G4**; pointer `docs/icml_portal_save_target.json`
...
| Gate | Status |
|------|--------|
| G0 mechanism unit tests | **PASS** (2026-08-03; … + Tick 32 per_run_venv / uv + Tick 34 SystemExit-safe probe + Tick 265 Astral bootstrap + Tick 266 runtime-deps) |
| G1 dry-run Condition D | **PASS** (2026-08-04) — `run_1401` + `test_cabs_inline_dry_run.py` |
| G2 smoke GPQA subset | **PREFLIGHT READY** (Tick 24/25 + Tick 32/34/265/266 `per_run_venv` + `runtime_deps`); **live** G2 still BLOCKED on **API keys** + HF_TOKEN / real diamond (see `docs/icml_portal_save_target.json`) |
