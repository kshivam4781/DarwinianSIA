...
| Gate | Status |
|------|--------|
| G0 mechanism unit tests | **PASS** (2026-08-03; + delay-all Tick 14; + compressed fitness Tick 16; + ε-greedy/live harvest Tick 17; + H5 protocol Tick 18–19; + directed explore Tick 20; + cost-to-threshold Tick 22; + post-steer case study Tick 23) |
| G1 dry-run Condition D | **PASS** (2026-08-04) — `run_1401` + `test_cabs_inline_dry_run.py` |
| G2 smoke GPQA subset | **HARNESS READY** (Tick 21: `prepare_gpqa_smoke_data.py` + CLI dry-run `run_1800`); **live** G2 still BLOCKED (no API keys; need real GPQA diamond; `python3.12-venv` installed this tick) |
