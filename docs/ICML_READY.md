...
- Cursor env: `.cursor/environment.json` (+ **uv**) + Tick 265 env `31d13f14-…` (build `ec92739d` SUCCEEDED + proposed) + **Astral uv bootstrap in G2/G3/G4**; pointer `docs/icml_portal_save_target.json`
- Per-run venv probe: `scripts/icml_env_checks.py` (`ensure_uv_on_path` Tick 265; Tick 32/34 SystemExit-safe probe)

## Gate tracker (Section 21.5)

| Gate | Status |
|------|--------|
| G0 mechanism unit tests | **PASS** (2026-08-03; … + Tick 32 per_run_venv / uv + Tick 34 SystemExit-safe probe + Tick 265 Astral bootstrap) |
| G1 dry-run Condition D | **PASS** (2026-08-04) — `run_1401` + `test_cabs_inline_dry_run.py` |
| G2 smoke GPQA subset | **PREFLIGHT READY** (Tick 24/25 + Tick 32/34/265 `per_run_venv` bootstrap); **live** G2 still BLOCKED on **API keys** + HF_TOKEN / real diamond (see `docs/icml_portal_save_target.json`) |
