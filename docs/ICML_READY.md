...
- Cursor env: `.cursor/environment.json` (+ **uv**) + Tick 40 draft `a1202e1f-…` (build `47d88b32` SUCCEEDED + proposed; must Portal Save onto automation — prior drafts not inherited); pointer `docs/icml_portal_save_target.json`
...
| G2 smoke GPQA subset | **PREFLIGHT READY** (Tick 24/25 + Tick 32/34 `per_run_venv`); **live** G2 still BLOCKED (no API keys; no HF_TOKEN / real diamond; need Portal Save uv env `a1202e1f-…` onto automation — see `docs/icml_portal_save_target.json`) |
