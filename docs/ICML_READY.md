...
_Tick 435: when already on tip but ``origin/<tip>`` is strictly ahead of HEAD, FF tip ← origin (preserving durable/companion dirt) before durable commit — pre-435 early-returned then NF-rejected tip push on a stale tip base. Tick 434 checkout-when-boot-behind + Tick 433 tip←HEAD sync when descendant + Tick 432 always→tip PR head remain. Tip PR #337 remains **MERGEABLE/CLEAN**. Live PRIMARY still blocked on NEBIUS + HF/CSV._
