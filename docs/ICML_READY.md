...
- Cursor env: `.cursor/environment.json` (+ **uv**) + Tick 230 env `31d13f14-…` (build `0f30c5bc` SUCCEEDED + proposed; must Portal Save onto automation — prior drafts not inherited); pointer `docs/icml_portal_save_target.json`
...
| G2 smoke GPQA subset | **PREFLIGHT READY** (Tick 24/25 + Tick 32/34 `per_run_venv`); **live** G2 still BLOCKED (no API keys; no HF_TOKEN / real diamond; need Portal Save uv env `31d13f14-…` / build `e1d81012` onto automation — see `docs/icml_portal_save_target.json`) |
