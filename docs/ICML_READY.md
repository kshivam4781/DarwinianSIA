...
- Evidence: offline synthetic pilot `1670–1674` vs `1680–1684` (Tick 17, ε-greedy + live bias harvest + delay-all + `max_gen=6`) — D gens30 wins **3/5** (B: 0), D final wins **5/5**, mean final gap ~**5.35pp**. Offline PRIMARY-shaped signal only — **not** live GPQA; leave unchecked for READY. Live → `docs/paper_artifacts.md` Table 1
...
- [x] Preferred-allele anchoring + singleton-bias skip (Tick 10) — updated Tick 17: outsiders preserved; ε-greedy explores
...
- [x] Documented case study (tie → contradiction → different DNA → fitness lift) with artifacts — offline dry-run `docs/case_study_offline.md` + `run_1683` (`stepwise` preferred → gen2 share **0.25**; lift +0.0869)
- [ ] Live API-run H2 DNA trait skew under contradiction bias
- Evidence: unit + dry-run G1 + scoped feedback + fitness-weighted order + preferred anchoring + bias-aware/delayed XO + tempered early mutation + delay-all mutation bias + compressed fitness scale + ε-greedy/live harvest + offline case study; live GPQA still pending (no API keys)

### 3. VALIDITY — H5
- [ ] Spearman ρ (`epistemic_value_t` vs `Δfitness_t+1`) > 0.3 on live / publishable runs
- Evidence: offline multi-seed Condition D `1680–1684` (`max_gen=6`) → ρ>0.3 on **2/5** seeds (0.3 / −0.3 / 0.3 / 0.6 / 0.6). Seed 22 regression under exploration noise. Still **not** live GPQA.
...
| Gate | Status |
|------|--------|
| G0 mechanism unit tests | **PASS** (2026-08-03; + delay-all Tick 14; + compressed fitness Tick 16; + ε-greedy/live harvest Tick 17) |
| G1 dry-run Condition D | **PASS** (2026-08-04) — `run_1401` + `test_cabs_inline_dry_run.py` |
| G2 smoke GPQA subset | BLOCKED (no API keys) |
| G3 pilot B vs D | Offline synthetic pilot refreshed (Tick 17; gens30 **3/5**); **live** G3 NOT STARTED |
| G4 5-seed + metrics | NOT STARTED (live) |
| G5 paper pack | PARTIAL (offline figs + case study + offline PRIMARY gens30; H5 soft); live pack NOT STARTED |
