...
- Evidence: offline synthetic pilot `1730–1734` vs `1740–1744` (Tick 18; Tick 17 ε-greedy path + H5 protocol) — D gens30 wins **3/5** (B: 0), D final wins **5/5**, mean final gap ~**5.35pp**. Offline PRIMARY-shaped signal only — **not** live GPQA; leave unchecked for READY. Live → `docs/paper_artifacts.md` Table 1
...
- [x] ε-greedy mutation + live population bias harvest (Tick 17) — escape suboptimal frozen contradiction pairs (`test_biased_mutate_epsilon_explores_outside_disputed_pool`, `test_mutation_bias_adopts_better_allele_from_latest_population`)
- [x] H5 protocol (Tick 18) — steered-window (`min_generation=2`) + population-mean Δfitness (`scripts/epistemic_results.py`)
- [x] Documented case study (tie → contradiction → different DNA → fitness lift) with artifacts — offline dry-run `docs/case_study_offline.md` + `run_1743` (`stepwise` preferred → gen2 share **0.25**; lift +0.0869)
- [ ] Live API-run H2 DNA trait skew under contradiction bias
- Evidence: unit + dry-run G1 + scoped feedback + fitness-weighted order + preferred anchoring + bias-aware/delayed XO + tempered early mutation + delay-all mutation bias + compressed fitness scale + ε-greedy/live harvest + H5 protocol + offline case study; live GPQA still pending (no API keys)

### 3. VALIDITY — H5
- [ ] Spearman ρ (`epistemic_value_t` vs `Δfitness_t+1`) > 0.3 on live / publishable runs
- Evidence: offline multi-seed Condition D `1740–1744` (`max_gen=6`) → ρ>0.3 on **4/5** seeds (0.0 / 0.8 / 0.4 / 0.8 / 0.6) using population-mean Δfitness and gen≥2 pairs (delay-all steered window). Seed 11 still fails. Still **not** live GPQA.
...
| Gate | Status |
|------|--------|
| G0 mechanism unit tests | **PASS** (2026-08-03; + delay-all Tick 14; + compressed fitness Tick 16; + ε-greedy/live harvest Tick 17; + H5 protocol Tick 18) |
| G1 dry-run Condition D | **PASS** (2026-08-04) — `run_1401` + `test_cabs_inline_dry_run.py` |
| G2 smoke GPQA subset | BLOCKED (no API keys) |
| G3 pilot B vs D | Offline synthetic pilot refreshed (Tick 18; gens30 **3/5**; H5 **4/5**); **live** G3 NOT STARTED |
| G4 5-seed + metrics | NOT STARTED (live) |
| G5 paper pack | PARTIAL (offline figs + case study + offline PRIMARY gens30 + offline H5 4/5); live pack NOT STARTED |
