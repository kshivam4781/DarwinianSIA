...
_Tick 439: prefer-richer prior_live gate merge on Tick 438 durable rebase — same-key gates keep executed / larger n_pairs / pass flags instead of always replayed local (closes thin-local wipe of onto G4 evidence). Tick 438 durable/paper-pack-only conflict auto-merge + Tick 437 NF rebase+retry remain. Tip PR #337 remains **MERGEABLE/CLEAN**. Live PRIMARY still blocked on NEBIUS + HF/CSV._
...
- [x] Tick 417 demote ICML_READY on refuse + ledger-skip always-refuse — `demote_icml_ready_file` rewrites disk `STATUS: IN_PROGRESS` on trust refuse; ledger-skip always refuses recomputed PRIMARY/H5/H2 fail (pipeline resume parity; pre-417 only refused when `meta.*_pass` claimed True)
- [x] Tick 418 tip↔main README conflict resolve — merge `origin/main` into tip; keep Linux/cloud `python3` block + portable Windows `cd path\to\SIA2`; tip PR #337 **MERGEABLE/CLEAN**
- [x] Tick 419 discard/tip-apply prior_live evidence race — fresh prior_live stash capture no longer fails discard or tip `--apply` on newly written evidence; pre-existing dirty evidence still blocks (Tick 390); `test_discard_ephemeral_ok_when_persist_writes_evidence`
- [x] Tick 420 auto prepare/commit prior_live evidence — `prepare_prior_live_evidence_for_tip_apply` parks sole dirty evidence into stash before discard; `commit_prior_live_evidence_if_dirty` commits after reinject + anti-churn; wired recover_tip/cron/boot_recover; `test_prepare_and_commit_prior_live_evidence_tip_apply_roundtrip`
...
- [x] Tick 425 tip-recover durable ledger commit+push — cron / boot_recover / recover_tip use `commit_durable_ledgers_on_tip_recover` (same commit+push+ahead-retry as post-live); closes commit-only mid-tick death after tip `--apply`; focused tests 5/5
- [x] Tick 426 paper-pack companion co-commit — post-G4 `apply_paper_pack` dirt (`paper_artifacts` / `ICML_READY` / Figs 1–2) co-committed+pushed with durable ledgers; pre-426 refused → spend/READY never reached origin; focused tests 6/6
- [x] Tick 427 tip-apply paper-pack companion park/reinject — prepare parks companions into gitignored `docs/icml_paper_pack_stash.json` (pre-427 refused as other non-ephemeral dirt → tip recover blocked mid-tick); reinject + durable commit after tip `--apply`; focused tests 9/9
- [x] Tick 428 consume durable stashes after successful push — `consume_durable_stashes_after_commit` unlinks paper-pack / budget / prior_live stashes after tip is on `origin` (keep on push fail); prevents stale READY reinject over demoted tip; focused durable/tip tests 20/20
- [x] Tick 429 consume only redundant durable stashes — keep unique mid-tick paper-pack/budget/prior_live stashes after failed reinject on tip-synced commit-noop; only unlink when payload matches HEAD; focused durable/tip tests 16/16
- [x] Tick 430 durable-stash redundancy vs committed HEAD — `_git_show_head_bytes`; reinject/staged-only WT match cannot wipe unique stash on commit-noop (`test_consume_keeps_stash_when_wt_matches_but_head_differs`)
- [x] Tick 431 durable push boot→tip — `resolve_push_branch_for_durable_ledgers` redirects unpushed greenfield boot / cloud-boot env to `tip_pr_commit_branch` (`test_resolve_push_branch_redirects_greenfield_boot_to_tip_pr`)
- [x] Tick 432 durable push always→tip PR head — when tip PR head known, always push to `tip_pr_commit_branch` even if `origin/<boot>` exists without capture
- [x] Tick 433 local tip ref sync before durable commit — fast-forward tip ← HEAD + checkout tip when HEAD is tip descendant (`test_ensure_local_tip_branch_syncs_boot_commit_onto_tip_ref`)
- [x] Tick 434 checkout tip when boot behind — preserve durable/companion dirt across tip checkout when HEAD is not a tip descendant; closes boot commit + non-FF tip push reject (`test_ensure_local_tip_checkouts_tip_when_boot_behind`)
- [x] Tick 435 FF tip when already-on-tip behind origin — preserve durable dirt; closes stale-tip-base commit + non-FF tip push (`test_ensure_local_tip_ffs_when_already_on_tip_behind_origin`)
- [x] Tick 436 fetch tip before Tick 435 FF — `fetch_origin_tip_for_durable_ledgers` refreshes `origin/<tip>` before ahead/checkout; closes stale remote-tracking skip-FF + NF tip push (`test_ensure_local_tip_fetches_before_ff_when_origin_tracking_stale`)
- [x] Tick 437 NF tip push rebase+retry — on non-fast-forward tip push, fetch + rebase onto `origin/<tip>` + retry once (never force); closes concurrent tip advance / diverged unique durable (`test_push_tip_rebases_after_nf_when_diverged_from_origin`)
- [x] Tick 438 durable rebase conflict merge — auto-merge durable/paper-pack-only conflicts (union `budget_spent`; merge `prior_live`; demote READY) + `rebase --continue`; closes concurrent tip VMs both writing spend → abort → local-only (`test_rebase_tip_merges_durable_budget_conflict`)
- [x] Tick 439 prefer-richer prior_live gate merge — same-key durable rebase merge keeps richer gate payload (executed / n_pairs / pass flags) instead of always replayed local; closes thin-local wipe of onto G4 evidence (`test_merge_prior_live_prefers_richer_onto_g4_over_thin_local`)
- [ ] Live API-run H2 DNA trait skew under contradiction bias
- Evidence: unit + dry-run G1 + scoped feedback + fitness-weighted order + preferred anchoring + bias-aware/delayed XO + tempered early mutation + delay-all mutation bias + compressed fitness scale + ε-greedy/live harvest + directed explore + H5 protocol + cost-to-threshold + **post-steering** offline case study + G2 preflight + diamond fetcher + G3 sequential runner + G4 5-seed runner + G4 paper-pack + unified live pipeline + Cursor env drafts + Tick 32 uv / per_run_venv + Tick 33 Portal Save pointer + Tick 34 SystemExit-safe probe + Tick 35–264 uv drafts + **Tick 265 Astral uv bootstrap** + **Tick 266 runtime-deps bootstrap** + **Tick 267 secrets-only gate verified** + **Tick 268 secrets-first status/unblock** + **Tick 269 tip lineage recover/refuse** + **Tick 270 main-boot bash tip recover** + **Tick 271 single cron entry** + **Tick 272 lineage chicken-egg tip pick** + **Tick 273 cron HF live gate** + **Tick 274 pipeline HF gate** + **Tick 275 G2/G3/G4 HF gate** + **Tick 276 preflight `--fetch-diamond` propagation** + **Tick 277 `.env` + CSV unlock** + **Tick 278 runner CSV autowire** + **Tick 286 ephemeral-dirt tip recover + zero ledger** + **Tick 287 host pandas-free GPQA eval_subset** + **Tick 303–306 shape locks / offline CLI defaults / G2+G3+G4 tip guards** + **Tick 422–439 durable commit/push/rebase/conflict-merge + prefer-richer prior_live**; live GPQA still pending (**API keys** + HF token / CSV; Portal Save optional for warm boots; **Tick 332** HUMAN_UNBLOCK chicken-egg also scans `cursor/bc-*`; **Tick 394** secrets-status auto-detect synthetic GPQA)
