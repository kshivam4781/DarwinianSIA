...
**Last updated:** 2026-08-07 (Section 21 ICML; Tick 43 re-link uv Cursor env draft `fbd56e14` + Portal Save target)  
...
| Cursor cloud environment (ICML live) | **PARTIAL** | Tick 43: draft `fbd56e14-…` + build `bld-20260807-a55ab7fc-…` **SUCCEEDED** + proposed (installs **uv**); pointer `docs/icml_portal_save_target.json`; prior drafts not inherited — need Portal Save onto automation + secrets |
...
| ICML B vs D multi-seed GPQA | **NOT DONE** | Blocked on Portal Save of uv-capable env `fbd56e14-…` onto automation + API keys + HF `Idavidrein/gpqa` accept; then `run_icml_live_pipeline.py --live --fetch-diamond` |
