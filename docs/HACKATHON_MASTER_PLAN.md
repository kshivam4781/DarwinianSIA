...
**Last updated:** 2026-08-13 (Section 21 ICML; Tick 115 re-link uv Cursor env draft `4be50240` + Portal Save target)  
...
| Cursor cloud environment (ICML live) | **PARTIAL** | Tick 115: draft `4be50240-…` + build `bld-20260813-427c3d44-…` **SUCCEEDED** + proposed (installs **uv** 0.12.3); pointer `docs/icml_portal_save_target.json`; prior drafts not inherited — need Portal Save onto automation + secrets |
...
| ICML B vs D multi-seed GPQA | **NOT DONE** | Blocked on Portal Save of uv-capable env `4be50240-…` onto automation + API keys + HF `Idavidrein/gpqa` accept; then `run_icml_live_pipeline.py --live --fetch-diamond` |
