...
**Last updated:** 2026-08-18 (Section 21 ICML; Tick 145 re-link uv Cursor env draft `9b30808d` + Portal Save target)  
...
| Cursor cloud environment (ICML live) | **PARTIAL** | Tick 145: draft `9b30808d-…` + build `bld-20260818-56e14f1c-…` **SUCCEEDED** + proposed (installs **uv** 0.12.5); pointer `docs/icml_portal_save_target.json`; prior drafts not inherited — need Portal Save onto automation + secrets |
...
| ICML B vs D multi-seed GPQA | **NOT DONE** | Blocked on Portal Save of uv-capable env `9b30808d-…` onto automation + API keys + HF `Idavidrein/gpqa` accept; then `run_icml_live_pipeline.py --live --fetch-diamond` |
