...
**Last updated:** 2026-08-19 (Section 21 ICML; Tick 153 re-link uv Cursor env draft `5a9477ec` + Portal Save target)  
...
| Cursor cloud environment (ICML live) | **PARTIAL** | Tick 153: draft `5a9477ec-…` + build `bld-20260819-6e55fea2-…` **SUCCEEDED** + proposed (installs **uv** 0.12.5); pointer `docs/icml_portal_save_target.json`; prior drafts not inherited — need Portal Save onto automation + secrets |
...
| ICML B vs D multi-seed GPQA | **NOT DONE** | Blocked on Portal Save of uv-capable env `5a9477ec-…` onto automation + API keys + HF `Idavidrein/gpqa` accept; then `run_icml_live_pipeline.py --live --fetch-diamond` |
