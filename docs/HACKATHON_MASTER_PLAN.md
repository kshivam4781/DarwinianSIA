...
**Last updated:** 2026-08-17 (Section 21 ICML; Tick 141 re-link uv Cursor env draft `a6aa98a7` + Portal Save target)  
...
| Cursor cloud environment (ICML live) | **PARTIAL** | Tick 141: draft `a6aa98a7-…` + build `bld-20260817-84e5d8a5-…` **SUCCEEDED** + proposed (installs **uv** 0.12.5); pointer `docs/icml_portal_save_target.json`; prior drafts not inherited — need Portal Save onto automation + secrets |
...
| ICML B vs D multi-seed GPQA | **NOT DONE** | Blocked on Portal Save of uv-capable env `a6aa98a7-…` onto automation + API keys + HF `Idavidrein/gpqa` accept; then `run_icml_live_pipeline.py --live --fetch-diamond` |
