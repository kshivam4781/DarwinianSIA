...
**Last updated:** 2026-08-12 (Section 21 ICML; Tick 96 re-link uv Cursor env draft `81e72868` + Portal Save target)  
...
| Cursor cloud environment (ICML live) | **PARTIAL** | Tick 96: draft `81e72868-…` + build `bld-20260812-6e157bcc-…` **SUCCEEDED** + proposed (installs **uv** 0.12.3); pointer `docs/icml_portal_save_target.json`; prior drafts not inherited — need Portal Save onto automation + secrets |
...
| ICML B vs D multi-seed GPQA | **NOT DONE** | Blocked on Portal Save of uv-capable env `81e72868-…` onto automation + API keys + HF `Idavidrein/gpqa` accept; then `run_icml_live_pipeline.py --live --fetch-diamond` |
...
**Portal Save re-link (2026-08-12 Tick 96):** Cron again booted `environment: null` (Tick 95 draft not attached to automation). Re-linked uv-capable personal draft `81e72868-9613-11f1-ba66-0e7d0216e441`; build `bld-20260812-6e157bcc-71ea-4234-be3e-838b85ce5a8d` **SUCCEEDED** (uv 0.12.3) + proposed; `docs/icml_portal_save_target.json` updated. User must Portal Save that draft onto automation `bf73dff3-…` and inject secrets, or every future cron will keep re-creating orphan drafts.
