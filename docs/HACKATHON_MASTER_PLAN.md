...
**Last updated:** 2026-08-19 (Section 21 ICML; Tick 159 re-link uv Cursor env draft `ac80f521` + Portal Save target)  
...
| Cursor cloud environment (ICML live) | **PARTIAL** | Tick 159: draft `ac80f521-…` + build `bld-20260819-aeb894b5-…` **SUCCEEDED** + proposed (installs **uv** 0.12.5); pointer `docs/icml_portal_save_target.json`; prior drafts not inherited — need Portal Save onto automation + secrets |
...
| ICML B vs D multi-seed GPQA | **NOT DONE** | Blocked on Portal Save of uv-capable env `ac80f521-…` onto automation + API keys + HF `Idavidrein/gpqa` accept; then `run_icml_live_pipeline.py --live --fetch-diamond` |
