...
**Last updated:** 2026-08-13 (Section 21 ICML; Tick 106 re-link uv Cursor env draft `7a0d714b` + Portal Save target)  
...
| Cursor cloud environment (ICML live) | **PARTIAL** | Tick 106: draft `7a0d714b-…` + build `bld-20260813-852aa860-…` **SUCCEEDED** + proposed (installs **uv** 0.12.3); pointer `docs/icml_portal_save_target.json`; prior drafts not inherited — need Portal Save onto automation + secrets |
...
| ICML B vs D multi-seed GPQA | **NOT DONE** | Blocked on Portal Save of uv-capable env `7a0d714b-…` onto automation + API keys + HF `Idavidrein/gpqa` accept; then `run_icml_live_pipeline.py --live --fetch-diamond` |
...
**Portal Save re-link (2026-08-13 Tick 106):** Cron again booted `environment: null` (Tick 105 draft not attached to automation). Re-linked uv-capable personal draft `7a0d714b-96bc-11f1-ba66-0e7d0216e441`; build `bld-20260813-852aa860-ac56-4296-af8f-33fe4d297b55` **SUCCEEDED** (uv 0.12.3) + proposed; `docs/icml_portal_save_target.json` updated. User must Portal Save that draft onto automation `bf73dff3-…` and inject secrets, or every future cron will keep re-creating orphan drafts.
