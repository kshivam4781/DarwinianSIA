...
**Last updated:** 2026-08-16 (Section 21 ICML; Tick 135 re-link uv Cursor env draft `793f5f75` + Portal Save target)  
...
| Cursor cloud environment (ICML live) | **PARTIAL** | Tick 135: draft `793f5f75-…` + build `bld-20260816-6f995d2d-…` **SUCCEEDED** + proposed (installs **uv** 0.12.5); pointer `docs/icml_portal_save_target.json`; prior drafts not inherited — need Portal Save onto automation + secrets |
...
| ICML B vs D multi-seed GPQA | **NOT DONE** | Blocked on Portal Save of uv-capable env `793f5f75-…` onto automation + API keys + HF `Idavidrein/gpqa` accept; then `run_icml_live_pipeline.py --live --fetch-diamond` |
