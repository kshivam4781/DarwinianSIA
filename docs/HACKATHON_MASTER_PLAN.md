...
**Last updated:** 2026-08-14 (Section 21 ICML; Tick 117 re-link uv Cursor env draft `be42444c` + Portal Save target)  
...
| Cursor cloud environment (ICML live) | **PARTIAL** | Tick 117: draft `be42444c-…` + build `bld-20260814-cc5e6bd7-…` **SUCCEEDED** + proposed (installs **uv** 0.12.4); pointer `docs/icml_portal_save_target.json`; prior drafts not inherited — need Portal Save onto automation + secrets |
...
| ICML B vs D multi-seed GPQA | **NOT DONE** | Blocked on Portal Save of uv-capable env `be42444c-…` onto automation + API keys + HF `Idavidrein/gpqa` accept; then `run_icml_live_pipeline.py --live --fetch-diamond` |
