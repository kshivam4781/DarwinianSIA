...
**Last updated:** 2026-08-12 (Section 21 ICML; Tick 104 re-link uv Cursor env draft `d5ce09b1` + Portal Save target)  
...
| Cursor cloud environment (ICML live) | **PARTIAL** | Tick 104: draft `d5ce09b1-…` + build `bld-20260812-2191a0c0-…` **SUCCEEDED** + proposed (installs **uv** 0.12.3); pointer `docs/icml_portal_save_target.json`; prior drafts not inherited — need Portal Save onto automation + secrets |
...
| ICML B vs D multi-seed GPQA | **NOT DONE** | Blocked on Portal Save of uv-capable env `d5ce09b1-…` onto automation + API keys + HF `Idavidrein/gpqa` accept; then `run_icml_live_pipeline.py --live --fetch-diamond` |
