...
**Last updated:** 2026-08-17 (Section 21 ICML; Tick 143 re-link uv Cursor env draft `14ed9320` + Portal Save target)  
...
| Cursor cloud environment (ICML live) | **PARTIAL** | Tick 143: draft `14ed9320-…` + build `bld-20260817-fb7f57ba-…` **SUCCEEDED** + proposed (installs **uv** 0.12.5); pointer `docs/icml_portal_save_target.json`; prior drafts not inherited — need Portal Save onto automation + secrets |
...
| ICML B vs D multi-seed GPQA | **NOT DONE** | Blocked on Portal Save of uv-capable env `14ed9320-…` onto automation + API keys + HF `Idavidrein/gpqa` accept; then `run_icml_live_pipeline.py --live --fetch-diamond` |
