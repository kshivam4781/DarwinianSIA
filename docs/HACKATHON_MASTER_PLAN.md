...
**Last updated:** 2026-08-15 (Section 21 ICML; Tick 129 re-link uv Cursor env draft `2acd30d9` + Portal Save target)  
...
| Cursor cloud environment (ICML live) | **PARTIAL** | Tick 129: draft `2acd30d9-…` + build `bld-20260815-d9c1598f-…` **SUCCEEDED** + proposed (installs **uv** 0.12.5); pointer `docs/icml_portal_save_target.json`; prior drafts not inherited — need Portal Save onto automation + secrets |
...
| ICML B vs D multi-seed GPQA | **NOT DONE** | Blocked on Portal Save of uv-capable env `2acd30d9-…` onto automation + API keys + HF `Idavidrein/gpqa` accept; then `run_icml_live_pipeline.py --live --fetch-diamond` |
