...
**Last updated:** 2026-08-07 (Section 21 ICML; Tick 48 re-link uv Cursor env draft `8433b834` + Portal Save target)  
...
| Cursor cloud environment (ICML live) | **PARTIAL** | Tick 48: draft `8433b834-…` + build `bld-20260807-d649e6ed-…` **SUCCEEDED** + proposed (installs **uv** 0.12.3); pointer `docs/icml_portal_save_target.json`; prior drafts not inherited — need Portal Save onto automation + secrets |
...
| ICML B vs D multi-seed GPQA | **NOT DONE** | Blocked on Portal Save of uv-capable env `8433b834-…` onto automation + API keys + HF `Idavidrein/gpqa` accept; then `run_icml_live_pipeline.py --live --fetch-diamond` |
