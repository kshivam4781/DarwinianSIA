...
**Last updated:** 2026-08-20 (Section 21 ICML; Tick 166 re-link uv Cursor env draft `dcebbcb8` + Portal Save target)  
...
| Cursor cloud environment (ICML live) | **PARTIAL** | Tick 166: draft `dcebbcb8-…` + build `bld-20260820-718ef891-…` **SUCCEEDED** + proposed (installs **uv** 0.12.5); pointer `docs/icml_portal_save_target.json`; prior drafts not inherited — need Portal Save onto automation + secrets |
...
| ICML B vs D multi-seed GPQA | **NOT DONE** | Blocked on Portal Save of uv-capable env `dcebbcb8-…` onto automation + API keys + HF `Idavidrein/gpqa` accept; then `run_icml_live_pipeline.py --live --fetch-diamond` |
