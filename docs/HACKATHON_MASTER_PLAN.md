...
**Last updated:** 2026-08-12 (Section 21 ICML; Tick 99 re-link uv Cursor env draft `70fcc83e` + Portal Save target)  
...
| Cursor cloud environment (ICML live) | **PARTIAL** | Tick 99: draft `70fcc83e-…` + build `bld-20260812-361b109b-…` **SUCCEEDED** + proposed (installs **uv** 0.12.3); pointer `docs/icml_portal_save_target.json`; prior drafts not inherited — need Portal Save onto automation + secrets |
...
| ICML B vs D multi-seed GPQA | **NOT DONE** | Blocked on Portal Save of uv-capable env `70fcc83e-…` onto automation + API keys + HF `Idavidrein/gpqa` accept; then `run_icml_live_pipeline.py --live --fetch-diamond` |
