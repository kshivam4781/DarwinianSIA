...
**Last updated:** 2026-08-10 (Section 21 ICML; Tick 82 re-link uv Cursor env draft `8a2353eb` + Portal Save target)  
...
| Cursor cloud environment (ICML live) | **PARTIAL** | Tick 82: draft `8a2353eb-…` + build `bld-20260810-c62a6167-…` **SUCCEEDED** + proposed (installs **uv** 0.12.3); pointer `docs/icml_portal_save_target.json`; prior drafts not inherited — need Portal Save onto automation + secrets |
...
| ICML B vs D multi-seed GPQA | **NOT DONE** | Blocked on Portal Save of uv-capable env `8a2353eb-…` onto automation + API keys + HF `Idavidrein/gpqa` accept; then `run_icml_live_pipeline.py --live --fetch-diamond` |
