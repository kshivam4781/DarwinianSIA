...
**Last updated:** 2026-08-06 (Section 21 ICML; Tick 34 re-link uv Cursor env draft + SystemExit-safe per_run_venv probe)  
...
| Cursor cloud environment (ICML live) | **PARTIAL** | Tick 34: draft `91d72d0c-…` + build `bld-20260806-262ebfe1-…` **SUCCEEDED** + proposed (installs **uv**); pointer `docs/icml_portal_save_target.json`; prior drafts not inherited — need Portal Save onto automation + secrets |
...
| ICML B vs D multi-seed GPQA | **NOT DONE** | Blocked on Portal Save of uv-capable env `91d72d0c-…` onto automation + API keys + HF `Idavidrein/gpqa` accept; then `run_icml_live_pipeline.py --live --fetch-diamond` |
