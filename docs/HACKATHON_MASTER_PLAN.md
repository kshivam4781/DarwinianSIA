...
**Last updated:** 2026-08-13 (Section 21 ICML; Tick 112 re-link uv Cursor env draft `d7e6f41e` + Portal Save target)  
...
| Cursor cloud environment (ICML live) | **PARTIAL** | Tick 112: draft `d7e6f41e-…` + build `bld-20260813-8e1487e8-…` **SUCCEEDED** + proposed (installs **uv** 0.12.3); pointer `docs/icml_portal_save_target.json`; prior drafts not inherited — need Portal Save onto automation + secrets |
...
| ICML B vs D multi-seed GPQA | **NOT DONE** | Blocked on Portal Save of uv-capable env `d7e6f41e-…` onto automation + API keys + HF `Idavidrein/gpqa` accept; then `run_icml_live_pipeline.py --live --fetch-diamond` |
