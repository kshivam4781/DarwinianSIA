...
**Last updated:** 2026-09-26 (Section 21 ICML; Tick 437 NF tip push rebase+retry; Tick 436 fetch tip before Tick 435 FF; Tick 435 FF tip when already-on-tip behind origin; Tick 434 checkout tip when boot behind; …)
...
**ICML Thesis 1 live default (Tick 288/289/316 — Nebius-only; Anthropic optional):**
...
└─────────────────────────────────────────┘
         ...
         Meta + Feedback + Target
         moonshotai/Kimi-K2.6
         (pydantic-ai meta; pay-per-token; no local GPU)
```

**Optional / historical (hackathon-era dual-vendor):** Anthropic Claude Haiku meta (`default-meta`) + Nebius Nemotron/Qwen target — only when intentionally selecting Claude meta (`ICML_META_AGENT_PROFILE=default-meta`). Do **not** provision Anthropic for ICML G2→G4 under default Nebius profiles.
...
**Budget split (hackathon-era shared pool; ICML Thesis 1 uses Nebius-only ~$20 ceiling — Section 21):**
- Reserve ~60% for SIA2 / CABS GPQA evidence (primary Track 3).
- Reserve ~40% for Darwinian subset runs (population × subset cases).
- ICML live G2→G4: spend is **Nebius** under default Kimi profiles; Anthropic only if Claude meta is selected.
...
**Gate (ICML Thesis 1 / Tick 289–328):** `NEBIUS_API_KEY` + (`HF_TOKEN` or local `gpqa_diamond.csv`) before paid G2→G4. Do **not** wait on Anthropic under default Nebius meta. Also **merge tip → `main`** so cron inherits ICML AGENTS/cron entry (Tick 327–328; secrets JSON `main_has_icml_tip`). See `docs/ICML_HUMAN_UNBLOCK.md`.
...
- Budget ceiling: **$200 total** unless user raises it (**ICML Thesis 1:** ~**$20** Nebius ceiling for G2→G4 — Section 21)
- If spend > $150: stop after 3 generations and submit with available data
- Always use `--no-web` on long runs
- **ICML live target + meta:** `kimi-nebius-target` + `kimi-nebius-pydantic-meta` (Nebius Kimi-K2.6; Tick 288/289/316). Do **not** default to Nemotron or Claude for ICML G2→G4. Nemotron/Claude remain optional alternates only.
...
| `HF_TOKEN` / diamond CSV | **ABSENT (cloud)** | Required for `--fetch-diamond` unless local `gpqa_diamond.csv` drop-path |
...
| ICML G3 full-pair metrics gate (Tick 411) | **DONE** | Live + ledger-skip + pipeline require all planned pairs before `score_pilot` / G4 advance (refuse partial pilot) |
...
| ICML park+commit budget_spent with prior_live (Tick 421) | **DONE** | Post-live dirty `docs/icml_budget_spent.json` no longer refuses Tick 420 prepare; parks both durable ledgers into gitignored stashes, reinjects after tip `--apply`, auto-commits onto tip; focused env-check tests 7/7 |
...
| ICML durable push always→tip PR head (Tick 432) | **DONE** | When tip PR head known, durable push **always** targets `tip_pr_commit_branch` (even if `origin/<boot>` exists without capture); closes re-park after accidental boot push; focused durable/push tests 10/10 |
...
| ICML G3 full-pair metrics gate (Tick 411) | **DONE** | Require all planned pairs before `score_pilot` / G4 advance |
...
| ICML tip PR anti-churn (Tick 337) | **DONE** | `prefer_tip_pr_commit_branch` + `tip_pr_commit_branch` / `tip_pr_anti_churn` in tip/secrets JSON; `scripts/icml_checkout_tip_pr_branch.sh`; human_next “do NOT open a new tip PR”; agents push onto MERGEABLE tip PR head |
| ICML cron tip PR anti-churn auto-checkout (Tick 338) | **DONE** | `icml_cron_entry.sh` auto-checkouts `tip_pr_commit_branch` after tip/secrets status write (closes Tick-337 gap: recover hard-resets SHA but kept greenfield branch name → new tip PR) |
| ICML tip recover --apply anti-churn checkout (Tick 339) | **DONE** | `icml_boot_recover.sh --apply` + `icml_recover_tip.py --apply` also auto-checkout `tip_pr_commit_branch` (closes Tick-338 chicken-egg-only path) |
| ICML open_git_pr never-omit-branch (Tick 340) | **DONE** | `docs/icml_open_git_pr.json` + tip/secrets `open_git_pr_branch` / `open_git_pr_never_omit_branch`; cron prints ALWAYS pass branch=; MCP omit → greenfield boot branch → new tip PR |
| ICML main-boot AGENTS chicken-egg bootstrap (Tick 341) | **DONE** | Branch `cursor/icml-main-agents-bootstrap` (1-file PR onto `main`; not a tip PR) — cron injects main AGENTS; bootstrap adds chicken-egg recover + dual unblock without requiring full tip #337 review first |
| ICML AGENTS bootstrap PR in human_next (Tick 342) | **DONE** | `resolve_icml_agents_bootstrap_pr` + `agents_bootstrap_pr_*` / `agents_bootstrap_merge_commands` on secrets/tip JSON; human_next leads with interim bootstrap when open (before full tip); lock test |
| ICML PRIMARY-first human_next (Tick 343) | **DONE** | When `fetch_diamond_ok` is false, secrets (+ HF accept) lead `human_next` / pipeline Next; tip/bootstrap merge follow (tip merge does not gate live); lock test |
| ICML secrets-first open_git_pr title (Tick 344) | **DONE** | `suggested_open_git_pr_title` + `tip_pr_title_stale` in `docs/icml_open_git_pr.json`; secrets-first title when diamond blocked; cron prints suggested title; lock test |
| ICML tip PR title edit commands (Tick 345) | **DONE** | `tip_pr_title_edit_commands` (`gh pr edit --title`) when `tip_pr_title_stale` — open_git_pr MCP does not rewrite GitHub titles on existing PRs; human_next + cron + JSON; lock test |
| ICML tip PR body-file refresh (Tick 346) | **DONE** | MCP also freezes GitHub body (PR #337 still Tick 336 desc); `suggested_open_git_pr_body` + `docs/icml_tip_pr_body.md`; edit cmds include `--body-file`; lock test |
| ICML tip_pr_body_stale independent of title (Tick 347) | **DONE** | `gh` fetches body; `parse_tick_from_pr_body` / `tip_pr_body_stale`; body-only `--body-file` paste when title already current; lock test |
| ICML open_git_pr pass description (Tick 348) | **DONE** | When `tip_pr_body_stale`, `open_git_pr_pass_description` + `open_git_pr_description_file`; agents pass `description=` from `docs/icml_tip_pr_body.md` (symmetric with title=); cron prints; lock test |
| ICML open_git_pr description inline (Tick 349) | **DONE** | Keep `open_git_pr_description` **inline** in `docs/icml_open_git_pr.json` (Tick 348 dropped body from JSON → agents skipped file read); md file still for `gh --body-file`; lock test |
| ICML open_git_pr call JSON (Tick 350) | **DONE** | `docs/icml_open_git_pr_call.json` atomic MCP `{branch, title, description}`; cron prints path; ephemeral + lock test |
| ICML tip PR anti-churn UNKNOWN mergeable (Tick 351) | **DONE** | `prefer_tip_pr_commit_branch` accepts UNKNOWN/null/empty mergeable; cron + checkout fall back to `tip_pr_head_ref`; lock test |
| ICML cloud_boot_branch open_git_pr warn (Tick 352) | **DONE** | `detect_cloud_boot_branch` + call/secrets JSON `cloud_boot_branch` / `omit_branch_opens_pr_on`; cron/`human_next` warn; Cloud Agent boot ≠ tip anti-churn |
| ICML cron early boot-branch capture (Tick 353) | **DONE** | `icml_cron_entry.sh` exports `ICML_CLOUD_BOOT_BRANCH` before tip recover / anti-churn (survives `ICML_CRON_REEXEC`); lock tests |
| ICML false-boot ignore + persist (Tick 354) | **DONE** | `detect_cloud_boot_branch` ignores env==tip; ephemeral `docs/icml_cloud_boot_branch.txt`; cron skips capture when already on tip |
| ICML cron no tip-boot-file clobber (Tick 355) | **DONE** | Preserved-env `elif` unsets `ICML_CLOUD_BOOT_BRANCH` when == tip; does not overwrite boot file with tip |
...
| ICML G3 full-pair metrics gate (Tick 411) | **DONE** | `g3_full_pairs_for_metrics` / `decide_g3_live_metrics_action`; refuse partial pilot → G4 |
| ICML G4 full-pair sidecar trust (Tick 412) | **DONE** | Ledger-skip + resume refuse `n_pairs < planned`; exit 4 on trust fail |
| ICML H5 planned-denominator VALIDITY (Tick 413) | **DONE** | `h5_validity_pass(..., planned_n=)` ≥3/5; refuse thin H5 READY |
| ICML PRIMARY recompute on sidecar trust (Tick 414) | **DONE** | Recompute PRIMARY from comparison on ledger-skip/resume; refuse false meta.primary_pass |
| ICML thin H2 MECHANISM refuse on sidecar trust (Tick 415) | **DONE** | Refuse thin/false meta.h2_pass on ledger-skip/resume; READY needs primary+h5+h2 |
...
# Or explicit live stack (same profiles the runners inject; python3 on Linux/cloud)
...
**Dual human unblock — secrets + merge tip→main (2026-09-03 Tick 327):** Live PRIMARY remains secrets-blocked, but cron also still boots **`main`**, which has hackathon-era `AGENTS.md` and **no** `docs/ICML_*` / `icml_cron_entry.sh` (300+ tip PRs unmerged). `docs/ICML_HUMAN_UNBLOCK.md` now leads with the dual action: (1) add NEBIUS + HF/CSV, (2) merge latest tip PR into `main`. Portal Save skipped. G2 dry-run `run_1910` green. STATUS remains IN_PROGRESS.
...
**Tip PR anti-churn (2026-09-04 Tick 337):** Tick 336 warned about churn, but each cron still opened a new tip PR on a greenfield branch, superseding the MERGEABLE one before humans could paste `gh`. Added `prefer_tip_pr_commit_branch` + tip/secrets fields `tip_pr_commit_branch` / `tip_pr_anti_churn`, `scripts/icml_checkout_tip_pr_branch.sh`, and human_next “do NOT open a new tip PR — checkout tip_pr_commit_branch”. This tick itself commits onto `cursor/icml-epistemic-results-f49c` (PR #337) rather than opening #338. Portal Save skipped. STATUS remains IN_PROGRESS.

**Cron tip PR anti-churn auto-checkout (2026-09-04 Tick 338):** Tick 337 left checkout as a manual script. `boot_recover --apply` only `git reset --hard` to the tip SHA while keeping the greenfield boot branch name, so agents still opened a new tip PR every cron. `icml_cron_entry.sh` now auto-checkouts `tip_pr_commit_branch` after writing tip/secrets status. Portal Save skipped. STATUS remains IN_PROGRESS.

**Tip recover --apply anti-churn checkout (2026-09-04 Tick 339):** Tick 338 only covered cron_entry. Chicken-egg `git show <tip>:…/icml_boot_recover.sh | bash -s -- --apply` (or `icml_recover_tip.py --apply` alone) still left greenfield branch names when agents committed before/without cron_entry. Both recover `--apply` paths now auto-checkout `tip_pr_commit_branch`. Portal Save skipped. STATUS remains IN_PROGRESS.

**open_git_pr never-omit-branch (2026-09-04 Tick 340):** Tick 337–339 checkout onto `tip_pr_commit_branch`, but `open_git_pr` MCP still defaults to the greenfield *boot* branch when `branch=` is omitted → new tip PR every cron. Added `docs/icml_open_git_pr.json` + tip/secrets fields `open_git_pr_branch` / `open_git_pr_never_omit_branch`, cron reminder, and human_next/AGENTS “NEVER omit branch=”. Portal Save skipped. STATUS remains IN_PROGRESS.

**Main-boot AGENTS chicken-egg bootstrap (2026-09-05 Tick 341):** Tip anti-churn works, but cron still injects hackathon-era `AGENTS.md` from `main` into cloud instructions while tip #337 (400+ commits) stays unmerged. Opened 1-file branch `cursor/icml-main-agents-bootstrap` (chicken-egg recover + dual unblock) — **not** a tip PR; tip anti-churn remains #337 / `f49c`. Portal Save skipped. STATUS remains IN_PROGRESS.

**AGENTS bootstrap PR in human_next (2026-09-05 Tick 342):** Tick 341 opened bootstrap PR #338, but secrets/tip `human_next` still led with tip #337 alone — operators reading cron logs missed the easier 1-file merge. `resolve_icml_agents_bootstrap_pr` + `_merge_agents_bootstrap_human_next` now put bootstrap URL + MERGEABLE + `gh` copy-paste first in `human_next` / pipeline Next, and tip/secrets JSON expose `agents_bootstrap_pr_*` / `agents_bootstrap_merge_commands`. Portal Save skipped. STATUS remains IN_PROGRESS.

**PRIMARY-first human_next (2026-09-05 Tick 343):** Tick 342 put bootstrap #338 first in cron `human_next`, so logs led with tip-merge hygiene while READY requires secrets → live PRIMARY. When `fetch_diamond_ok` is false, secrets (+ HF accept) now lead; tip/bootstrap merge follow. When secrets+HF are OK and main lacks tip, Tick 342 bootstrap-first order is unchanged. Portal Save skipped. STATUS remains IN_PROGRESS.

**Secrets-first open_git_pr title (2026-09-05 Tick 344):** Tip PR #337 stayed titled Tick 336 through Ticks 337–343, so among 300+ draft tip PRs it looked superseded. `docs/icml_open_git_pr.json` now exposes `tip_pr_title_stale` + `suggested_open_git_pr_title` (NEBIUS+HF lead when `fetch_diamond_ok` is false). Cron prints the suggested title; agents must pass `title=` with `branch=` on `open_git_pr`. Portal Save skipped. STATUS remains IN_PROGRESS.

**Tip PR title edit commands (2026-09-05 Tick 345):** Tick 344 found `open_git_pr` MCP does **not** rewrite GitHub titles on existing tip PRs (title stayed Tick 336 even when agents passed `title=`). When `tip_pr_title_stale`, secrets/tip/`open_git_pr` JSON + cron `human_next` now expose `tip_pr_title_edit_commands` — copy-paste `gh pr edit <N> --repo kshivam4781/DarwinianSIA --title '…'` (secrets-first when diamond blocked). Portal Save skipped. STATUS remains IN_PROGRESS.

**Tip PR body-file refresh (2026-09-05 Tick 346):** Tick 345 covered title-only paste, but `gh pr view 337` still showed a **Tick 336 body** after Ticks 337–345 — `open_git_pr` MCP does not rewrite title *or* body. When stale, edit commands now include `--body-file docs/icml_tip_pr_body.md` (secrets-first dual-unblock). Portal Save skipped. STATUS remains IN_PROGRESS.

**tip_pr_body_stale independent of title (2026-09-05 Tick 347):** Tick 346 gated `--body-file` on `tip_pr_title_stale` only — a title-only `gh pr edit` dropped the body paste while GitHub body stayed Tick 336. Now `gh pr list` fetches `body`; `parse_tick_from_pr_body` / `tip_pr_body_stale` drive body-file independently (body-only paste when title is already current). Portal Save skipped. STATUS remains IN_PROGRESS.

**open_git_pr pass description (2026-09-05 Tick 348):** Tick 344 told agents to pass `title=` but not `description=`. When `tip_pr_body_stale`, `docs/icml_open_git_pr.json` now exposes `open_git_pr_pass_description` / `open_git_pr_description_file`; agents must pass `description=` from `docs/icml_tip_pr_body.md` (symmetric with `title=`). MCP may still leave GitHub body frozen on existing PRs — human `gh pr edit --body-file` remains the refresh path. Portal Save skipped. STATUS remains IN_PROGRESS.

**open_git_pr description inline (2026-09-05 Tick 349):** Tick 348 pointed at `docs/icml_tip_pr_body.md` but **dropped** the body string from `docs/icml_open_git_pr.json`, so agents skipped the extra read and never passed `description=`. `write_icml_open_git_pr_hint` now keeps `open_git_pr_description` inline (md file retained for `gh --body-file`). Portal Save skipped. STATUS remains IN_PROGRESS.

**ICML open_git_pr call JSON (Tick 350):** `write_icml_open_git_pr_hint` also writes `docs/icml_open_git_pr_call.json` with exact MCP args `{branch, title, description}` so agents pass all three verbatim (avoids hunting fields in the large hint JSON). Cron prints the call-file path; ephemeral set extended. Portal Save skipped. STATUS remains IN_PROGRESS.

**ICML tip PR anti-churn UNKNOWN mergeable (Tick 351):** `prefer_tip_pr_commit_branch` previously required exact `MERGEABLE`; GitHub null/`UNKNOWN` left `tip_pr_commit_branch` empty and cron skipped anti-churn on greenfield boots (this tick booted `8c85`). Now accepts UNKNOWN/null/empty unless CONFLICTING/DIRTY; cron + `icml_checkout_tip_pr_branch.sh` fall back to `tip_pr_head_ref`. Portal Save skipped. STATUS remains IN_PROGRESS.

**ICML cloud_boot_branch open_git_pr warn (Tick 352):** This cron booted greenfield `cursor/icml-epistemic-results-1fa6` while tip PR #337 is `f49c`. After anti-churn checkout, `open_git_pr` MCP still defaults to the *boot* branch when `branch=` is omitted — opening a new tip PR. `detect_cloud_boot_branch` + call/secrets JSON now record `cloud_boot_branch` / `omit_branch_opens_pr_on`; cron/`human_next` warn on mismatch. Cloud Agent “correct working branch” does **not** override tip anti-churn. Portal Save skipped. STATUS remains IN_PROGRESS.

**ICML cron early boot-branch capture (Tick 353):** This cron booted greenfield `cursor/icml-epistemic-results-e24a`. Tick 352 relied on post-checkout reflog for `cloud_boot_branch`; after `git reset --hard` + tip anti-churn the reflog can miss or stale-pick the boot. `icml_cron_entry.sh` now exports `ICML_CLOUD_BOOT_BRANCH` from `git branch --show-current` *before* tip recover (preserved across `ICML_CRON_REEXEC`). Portal Save skipped. STATUS remains IN_PROGRESS.

**ICML false-boot ignore + persist (Tick 354):** This cron booted greenfield `cursor/icml-epistemic-results-6a00`. Agents that check out tip `f49c` *before* running cron caused Tick 353 to export tip as “boot” (env==tip), silencing the open_git_pr mismatch warn while MCP still defaults omit-branch to the real boot. `detect_cloud_boot_branch` now ignores env==tip, persists true boot to ephemeral `docs/icml_cloud_boot_branch.txt`, and cron skips capture when already on tip (reflog / boot file). Portal Save skipped. STATUS remains IN_PROGRESS.

**ICML cron no tip-boot-file clobber (Tick 355):** This cron booted greenfield `cursor/icml-epistemic-results-3b0f`. Tick 354 left a hole: when `ICML_CLOUD_BOOT_BRANCH` was *already* set to tip (agent false export / prior re-exec), the preserved-env `elif` still `printf`’d tip into `docs/icml_cloud_boot_branch.txt`, clobbering the real boot. Cron now unsets env==tip and keeps the boot file / reflog. Portal Save skipped. STATUS remains IN_PROGRESS.

**ICML boot-file gitignore + discard survive (Tick 356):** This cron booted greenfield `cursor/icml-epistemic-results-5fe4`. Tick 354–355 made `docs/icml_cloud_boot_branch.txt` the durable boot fallback, but it was listed in `EPHEMERAL_ICML_RELPATHS`, so `discard_ephemeral_icml_dirt` (tip `--apply`) unlinked it as "untracked removed". Also not gitignored → risk of committing a boot name onto tip. Now gitignored + excluded from ephemeral discard. Portal Save skipped. STATUS remains IN_PROGRESS.
...
**Tip-apply gitignore-lag durables (Tick 391):** Cron persists `docs/icml_cloud_boot_branch.txt` *before* tip recover; on greenfield/main boots without tip `.gitignore`, porcelain shows the boot file (and call JSON / prior_live stash) as dirty and refused `--apply` (Tick 390 chicken-egg hole). `TIP_APPLY_GITIGNORE_LAG_RELPATHS` + `is_tip_apply_ignored_dirty` filter those paths in `tip_apply_blocking_dirty_paths` / `discard_ephemeral_icml_dirt` / `icml_recover_tip.py --apply` (shared helper); also fixes Tick 390 undefined `evidence_norm` in post-discard remaining check. Evidence still blocks (Tick 390 ledger parity). Live still blocked on NEBIUS + HF/CSV. STATUS remains IN_PROGRESS.
...
**Cron secrets refresh after preflight (Tick 395):** Tick 394 auto-detect still left `gpqa_is_synthetic=null` on greenfield boots because cron wrote secrets **before** G2 preflight `ensure_smoke_layout` materializes `data/`. Cron now refreshes secrets after preflight and prints `human_next` afterward so the synthetic-diamond blocker is visible. Live still blocked on NEBIUS + HF/CSV. STATUS remains IN_PROGRESS.
...
**G3 full-pair metrics gate (Tick 411):** Tick 410 closed G4 partial equal-pair paper pack, but G3 live still scored when `len(B)==len(D)` only — a mid-G3 sia-exit leaving 1 of 2 pairs could write a partial `comparison` that pipeline `g3_pilot_promising` would treat as a full pilot → ~$14 G4 burn. Now `g3_full_pairs_for_metrics` / `decide_g3_live_metrics_action` require all planned pairs before `score_pilot`; ledger-skip + `load_g3_metrics_for_g4` refuse sidecar `n_pairs < planned`. Live still blocked on NEBIUS + HF/CSV. STATUS remains IN_PROGRESS.

**G4 full-pair sidecar trust (Tick 412):** Tick 411 closed G3 sidecar `n_pairs < planned`, but G4 ledger-skip / pipeline resume still trusted a partial gate4 sidecar (e.g. `n_pairs=1` with `ready_status=READY`) and exited 0 — poisoning READY trust after mid-abort. Now `refresh_paper_pack_on_ledger_skip` + `refresh_g4_paper_pack_on_resume` refuse sidecar `n_pairs < planned`; direct G4 ledger-skip exits 4 on trust fail; pipeline resume returns 4 on refuse notes. Live still blocked on NEBIUS + HF/CSV. STATUS remains IN_PROGRESS.
...
**Park+commit budget_spent with prior_live (Tick 421):** Tick 420 still left post-live tip `--apply` blocked whenever `docs/icml_budget_spent.json` was dirty alongside evidence (prepare refused — budget counted as other non-ephemeral dirt). `prepare_prior_live_evidence_for_tip_apply` now parks both durable ledgers into gitignored stashes (`docs/icml_budget_spent_stash.json` + prior_live stash) and restores HEAD; `reinject_budget_spent_stash` + `commit_prior_live_evidence_if_dirty` reinject and auto-commit both after tip `--apply` + anti-churn. Wired recover_tip / cron / boot_recover. Focused env-check tests **7** green. Live still blocked on secrets. STATUS remains IN_PROGRESS.
...
**Paper-pack companion co-commit (Tick 426):** After paid G4, `apply_paper_pack` dirties `docs/paper_artifacts.md`, `docs/ICML_READY.md`, and Figs 1–2. Pre-426 `commit_durable_ledgers_after_live` treated those as blocking non-ephemeral dirt → refuse → spend/READY/Live Tables never reached `origin`. Durable commit now co-commits the paper-pack whitelist (still refuses unrelated code dirt) and pushes. Focused tests **6** green. Live still blocked on secrets. STATUS remains IN_PROGRESS.
...
**Durable local tip ref sync (Tick 433):** Tick 432 pushes `HEAD:refs/heads/<tip>`, but a durable commit while still on a greenfield **boot** branch name could leave the local tip ref at the pre-commit tip SHA — later `git checkout <tip>` dropped unpushed spend/READY; ahead from that old tip HEAD was 0 → consume wiped reinject stashes. `ensure_local_tip_branch_for_durable_ledgers` now fast-forwards tip ← HEAD (never rewind) + checkouts tip before commit/push; `tip_commits_ahead_of_origin` also counts the tip ref. Focused durable/tip tests **14** green. Live still blocked on secrets. STATUS remains IN_PROGRESS.
