...
**Last updated:** 2026-08-19 (Section 21 ICML; Tick 156 re-link uv Cursor env draft `7f492c98` + Portal Save target)  
...
| Cursor cloud environment (ICML live) | **PARTIAL** | Tick 156: draft `7f492c98-…` + build `bld-20260819-2526ce25-…` **SUCCEEDED** + proposed (installs **uv** 0.12.5); pointer `docs/icml_portal_save_target.json`; prior drafts not inherited — need Portal Save onto automation + secrets |
...
| ICML B vs D multi-seed GPQA | **NOT DONE** | Blocked on Portal Save of uv-capable env `7f492c98-…` onto automation + API keys + HF `Idavidrein/gpqa` accept; then `run_icml_live_pipeline.py --live --fetch-diamond` |
