...
**Last updated:** 2026-08-14 (Section 21 ICML; Tick 120 re-link uv Cursor env draft `58f2651d` + Portal Save target)  
...
| Cursor cloud environment (ICML live) | **PARTIAL** | Tick 120: draft `58f2651d-…` + build `bld-20260814-8455afe8-…` **SUCCEEDED** + proposed (installs **uv** 0.12.5); pointer `docs/icml_portal_save_target.json`; prior drafts not inherited — need Portal Save onto automation + secrets |
...
| ICML B vs D multi-seed GPQA | **NOT DONE** | Blocked on Portal Save of uv-capable env `58f2651d-…` onto automation + API keys + HF `Idavidrein/gpqa` accept; then `run_icml_live_pipeline.py --live --fetch-diamond` |
