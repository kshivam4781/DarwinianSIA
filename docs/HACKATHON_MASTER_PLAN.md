...
**Last updated:** 2026-08-06 (Section 21 ICML; Tick 31 re-linked Cursor env on greenfield cron)  
...
| Cursor cloud environment (ICML live) | **PARTIAL** | Tick 31: re-linked draft `4b2bb39a-…` (Tick 30 `0ed19edd-…` not inherited by automation); build `bld-20260806-933779ed-…` **SUCCEEDED** + proposed; need Portal Save onto automation + secrets |
| ICML B vs D multi-seed GPQA | **NOT DONE** | Env re-linked (Tick 31); blocked on Portal Save onto automation + API keys + HF `Idavidrein/gpqa` accept; then `run_icml_live_pipeline.py --live --fetch-diamond` |
