...
**Last updated:** 2026-08-20 (Section 21 ICML; Tick 168 re-link uv Cursor env draft `95537bcc` + Portal Save target)  
...
| Cursor cloud environment (ICML live) | **PARTIAL** | Tick 168: draft `95537bcc-…` + build `bld-20260820-c5f0575a-…` **SUCCEEDED** + proposed (installs **uv** 0.12.5); pointer `docs/icml_portal_save_target.json`; prior drafts not inherited — need Portal Save onto automation + secrets |
...
| ICML B vs D multi-seed GPQA | **NOT DONE** | Blocked on Portal Save of uv-capable env `95537bcc-…` onto automation + API keys + HF `Idavidrein/gpqa` accept; then `run_icml_live_pipeline.py --live --fetch-diamond` |
