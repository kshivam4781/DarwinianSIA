...
**Last updated:** 2026-08-11 (Section 21 ICML; Tick 84 re-link uv Cursor env draft `c2580665` + Portal Save target)  
...
| Cursor cloud environment (ICML live) | **PARTIAL** | Tick 84: draft `c2580665-…` + build `bld-20260811-20b04108-…` **SUCCEEDED** + proposed (installs **uv** 0.12.3); pointer `docs/icml_portal_save_target.json`; prior drafts not inherited — need Portal Save onto automation + secrets |
...
| ICML B vs D multi-seed GPQA | **NOT DONE** | Blocked on Portal Save of uv-capable env `c2580665-…` onto automation + API keys + HF `Idavidrein/gpqa` accept; then `run_icml_live_pipeline.py --live --fetch-diamond` |
