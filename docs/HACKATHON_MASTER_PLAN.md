...
**Last updated:** 2026-08-09 (Section 21 ICML; Tick 69 re-link uv Cursor env draft `af3715f5` + Portal Save target)  
...
| Cursor cloud environment (ICML live) | **PARTIAL** | Tick 69: draft `af3715f5-…` + build `bld-20260809-8710d0db-…` **SUCCEEDED** + proposed (installs **uv** 0.12.3); pointer `docs/icml_portal_save_target.json`; prior drafts not inherited — need Portal Save onto automation + secrets |
...
| ICML B vs D multi-seed GPQA | **NOT DONE** | Blocked on Portal Save of uv-capable env `af3715f5-…` onto automation + API keys + HF `Idavidrein/gpqa` accept; then `run_icml_live_pipeline.py --live --fetch-diamond` |
