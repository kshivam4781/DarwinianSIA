...
**Last updated:** 2026-08-18 (Section 21 ICML; Tick 149 re-link uv Cursor env draft `8fdd51f9` + Portal Save target)  
...
| Cursor cloud environment (ICML live) | **PARTIAL** | Tick 149: draft `8fdd51f9-…` + build `bld-20260818-4dbf76d8-…` **SUCCEEDED** + proposed (installs **uv** 0.12.5); pointer `docs/icml_portal_save_target.json`; prior drafts not inherited — need Portal Save onto automation + secrets |
...
| ICML B vs D multi-seed GPQA | **NOT DONE** | Blocked on Portal Save of uv-capable env `8fdd51f9-…` onto automation + API keys + HF `Idavidrein/gpqa` accept; then `run_icml_live_pipeline.py --live --fetch-diamond` |
