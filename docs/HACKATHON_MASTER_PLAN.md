...
**Last updated:** 2026-08-08 (Section 21 ICML; Tick 54 re-link uv Cursor env draft `3b58dff6` + Portal Save target)  
...
| Cursor cloud environment (ICML live) | **PARTIAL** | Tick 54: draft `3b58dff6-…` + build `bld-20260808-14292e5c-…` **SUCCEEDED** + proposed (installs **uv** 0.12.3); pointer `docs/icml_portal_save_target.json`; prior drafts not inherited — need Portal Save onto automation + secrets |
...
| ICML B vs D multi-seed GPQA | **NOT DONE** | Blocked on Portal Save of uv-capable env `3b58dff6-…` onto automation + API keys + HF `Idavidrein/gpqa` accept; then `run_icml_live_pipeline.py --live --fetch-diamond` |
