...
**Last updated:** 2026-08-18 (Section 21 ICML; Tick 146 re-link uv Cursor env draft `362bb30f` + Portal Save target)  
...
| Cursor cloud environment (ICML live) | **PARTIAL** | Tick 146: draft `362bb30f-…` + build `bld-20260818-8f8a4648-…` **SUCCEEDED** + proposed (installs **uv** 0.12.5); pointer `docs/icml_portal_save_target.json`; prior drafts not inherited — need Portal Save onto automation + secrets |
...
| ICML B vs D multi-seed GPQA | **NOT DONE** | Blocked on Portal Save of uv-capable env `362bb30f-…` onto automation + API keys + HF `Idavidrein/gpqa` accept; then `run_icml_live_pipeline.py --live --fetch-diamond` |
