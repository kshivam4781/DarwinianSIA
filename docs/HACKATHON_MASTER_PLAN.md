...
**Last updated:** 2026-08-20 (Section 21 ICML; Tick 169 re-link uv Cursor env draft `3475a2ec` + Portal Save target)  
...
| Cursor cloud environment (ICML live) | **PARTIAL** | Tick 169: draft `3475a2ec-…` + build `bld-20260820-84bf37db-…` **SUCCEEDED** + proposed (installs **uv** 0.12.5); pointer `docs/icml_portal_save_target.json`; prior drafts not inherited — need Portal Save onto automation + secrets |
...
| ICML B vs D multi-seed GPQA | **NOT DONE** | Blocked on Portal Save of uv-capable env `3475a2ec-…` onto automation + API keys + HF `Idavidrein/gpqa` accept; then `run_icml_live_pipeline.py --live --fetch-diamond` |
