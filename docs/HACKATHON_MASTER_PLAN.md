...
**Last updated:** 2026-08-12 (Section 21 ICML; Tick 100 re-link uv Cursor env draft `c2ad6d68` + Portal Save target)  
...
| Cursor cloud environment (ICML live) | **PARTIAL** | Tick 100: draft `c2ad6d68-…` + build `bld-20260812-490aa59b-…` **SUCCEEDED** + proposed (installs **uv** 0.12.3); pointer `docs/icml_portal_save_target.json`; prior drafts not inherited — need Portal Save onto automation + secrets |
...
| ICML B vs D multi-seed GPQA | **NOT DONE** | Blocked on Portal Save of uv-capable env `c2ad6d68-…` onto automation + API keys + HF `Idavidrein/gpqa` accept; then `run_icml_live_pipeline.py --live --fetch-diamond` |
