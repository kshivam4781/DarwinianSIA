...
**Last updated:** 2026-08-15 (Section 21 ICML; Tick 124 re-link uv Cursor env draft `cfa45bdf` + Portal Save target)  
...
| Cursor cloud environment (ICML live) | **PARTIAL** | Tick 124: draft `cfa45bdf-…` + build `bld-20260815-ac69edae-…` **SUCCEEDED** + proposed (installs **uv** 0.12.5); pointer `docs/icml_portal_save_target.json`; prior drafts not inherited — need Portal Save onto automation + secrets |
...
| ICML B vs D multi-seed GPQA | **NOT DONE** | Blocked on Portal Save of uv-capable env `cfa45bdf-…` onto automation + API keys + HF `Idavidrein/gpqa` accept; then `run_icml_live_pipeline.py --live --fetch-diamond` |
