...
**Last updated:** 2026-08-07 (Section 21 ICML; Tick 47 re-link uv Cursor env draft `eabae511` + Portal Save target)  
...
| Cursor cloud environment (ICML live) | **PARTIAL** | Tick 47: draft `eabae511-…` + build `bld-20260807-b06442a0-…` **SUCCEEDED** + proposed (installs **uv** 0.12.3); pointer `docs/icml_portal_save_target.json`; prior drafts not inherited — need Portal Save onto automation + secrets |
...
| ICML B vs D multi-seed GPQA | **NOT DONE** | Blocked on Portal Save of uv-capable env `eabae511-…` onto automation + API keys + HF `Idavidrein/gpqa` accept; then `run_icml_live_pipeline.py --live --fetch-diamond` |
