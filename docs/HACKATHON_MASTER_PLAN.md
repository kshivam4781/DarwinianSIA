...
**Last updated:** 2026-08-13 (Section 21 ICML; Tick 107 re-link uv Cursor env draft `eccd72e0` + Portal Save target)  
...
| Cursor cloud environment (ICML live) | **PARTIAL** | Tick 107: draft `eccd72e0-…` + build `bld-20260813-55688c31-…` **SUCCEEDED** + proposed (installs **uv** 0.12.3); pointer `docs/icml_portal_save_target.json`; prior drafts not inherited — need Portal Save onto automation + secrets |
...
| ICML B vs D multi-seed GPQA | **NOT DONE** | Blocked on Portal Save of uv-capable env `eccd72e0-…` onto automation + API keys + HF `Idavidrein/gpqa` accept; then `run_icml_live_pipeline.py --live --fetch-diamond` |
