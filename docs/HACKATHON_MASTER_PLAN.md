...
**Last updated:** 2026-08-13 (Section 21 ICML; Tick 113 re-link uv Cursor env draft `4b6c5dd1` + Portal Save target)  
...
| Cursor cloud environment (ICML live) | **PARTIAL** | Tick 113: draft `4b6c5dd1-…` + build `bld-20260813-79322e5f-…` **SUCCEEDED** + proposed (installs **uv** 0.12.3); pointer `docs/icml_portal_save_target.json`; prior drafts not inherited — need Portal Save onto automation + secrets |
...
| ICML B vs D multi-seed GPQA | **NOT DONE** | Blocked on Portal Save of uv-capable env `4b6c5dd1-…` onto automation + API keys + HF `Idavidrein/gpqa` accept; then `run_icml_live_pipeline.py --live --fetch-diamond` |
