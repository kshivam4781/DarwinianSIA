...
**Last updated:** 2026-08-07 (Section 21 ICML; Tick 42 re-link uv Cursor env draft `44dc791a` + Portal Save target)  
...
| Cursor cloud environment (ICML live) | **PARTIAL** | Tick 42: draft `44dc791a-…` + build `bld-20260807-ef042f32-…` **SUCCEEDED** + proposed (installs **uv**); pointer `docs/icml_portal_save_target.json`; prior drafts not inherited — need Portal Save onto automation + secrets |
...
| ICML B vs D multi-seed GPQA | **NOT DONE** | Blocked on Portal Save of uv-capable env `44dc791a-…` onto automation + API keys + HF `Idavidrein/gpqa` accept; then `run_icml_live_pipeline.py --live --fetch-diamond` |
