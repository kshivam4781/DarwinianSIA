...
**Last updated:** 2026-08-10 (Section 21 ICML; Tick 77 re-link uv Cursor env draft `6c885367` + Portal Save target)  
...
| Cursor cloud environment (ICML live) | **PARTIAL** | Tick 77: draft `6c885367-…` + build `bld-20260810-760dbe3c-…` **SUCCEEDED** + proposed (installs **uv** 0.12.3); pointer `docs/icml_portal_save_target.json`; prior drafts not inherited — need Portal Save onto automation + secrets |
...
| ICML B vs D multi-seed GPQA | **NOT DONE** | Blocked on Portal Save of uv-capable env `6c885367-…` onto automation + API keys + HF `Idavidrein/gpqa` accept; then `run_icml_live_pipeline.py --live --fetch-diamond` |
