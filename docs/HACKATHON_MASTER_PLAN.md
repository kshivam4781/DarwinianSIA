...
**Last updated:** 2026-08-12 (Section 21 ICML; Tick 95 re-link uv Cursor env draft `bb5e7e76` + Portal Save target)  
...
| Cursor cloud environment (ICML live) | **PARTIAL** | Tick 95: draft `bb5e7e76-…` + build `bld-20260812-88c48096-…` **SUCCEEDED** + proposed (installs **uv** 0.12.3); pointer `docs/icml_portal_save_target.json`; prior drafts not inherited — need Portal Save onto automation + secrets |
...
| ICML B vs D multi-seed GPQA | **NOT DONE** | Blocked on Portal Save of uv-capable env `bb5e7e76-…` onto automation + API keys + HF `Idavidrein/gpqa` accept; then `run_icml_live_pipeline.py --live --fetch-diamond` |
