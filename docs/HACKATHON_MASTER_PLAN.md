...
**Last updated:** 2026-08-13 (Section 21 ICML; Tick 108 re-link uv Cursor env draft `a88df79f` + Portal Save target)  
...
| Cursor cloud environment (ICML live) | **PARTIAL** | Tick 108: draft `a88df79f-…` + build `bld-20260813-cebb7bd7-…` **SUCCEEDED** + proposed (installs **uv** 0.12.3); pointer `docs/icml_portal_save_target.json`; prior drafts not inherited — need Portal Save onto automation + secrets |
...
| ICML B vs D multi-seed GPQA | **NOT DONE** | Blocked on Portal Save of uv-capable env `a88df79f-…` onto automation + API keys + HF `Idavidrein/gpqa` accept; then `run_icml_live_pipeline.py --live --fetch-diamond` |
