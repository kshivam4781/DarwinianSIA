...
**Last updated:** 2026-08-11 (Section 21 ICML; Tick 88 re-link uv Cursor env draft `b1e29669` + Portal Save target)  
...
| Cursor cloud environment (ICML live) | **PARTIAL** | Tick 88: draft `b1e29669-…` + build `bld-20260811-768b7912-…` **SUCCEEDED** + proposed (installs **uv** 0.12.3); pointer `docs/icml_portal_save_target.json`; prior drafts not inherited — need Portal Save onto automation + secrets |
...
| ICML B vs D multi-seed GPQA | **NOT DONE** | Blocked on Portal Save of uv-capable env `b1e29669-…` onto automation + API keys + HF `Idavidrein/gpqa` accept; then `run_icml_live_pipeline.py --live --fetch-diamond` |
