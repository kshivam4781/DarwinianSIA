...
**Last updated:** 2026-08-15 (Section 21 ICML; Tick 127 re-link uv Cursor env draft `54dea794` + Portal Save target)  
...
| Cursor cloud environment (ICML live) | **PARTIAL** | Tick 127: draft `54dea794-…` + build `bld-20260815-d5e3334b-…` **SUCCEEDED** + proposed (installs **uv** 0.12.5); pointer `docs/icml_portal_save_target.json`; prior drafts not inherited — need Portal Save onto automation + secrets |
...
| ICML B vs D multi-seed GPQA | **NOT DONE** | Blocked on Portal Save of uv-capable env `54dea794-…` onto automation + API keys + HF `Idavidrein/gpqa` accept; then `run_icml_live_pipeline.py --live --fetch-diamond` |
