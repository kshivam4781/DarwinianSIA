...
**Last updated:** 2026-08-14 (Section 21 ICML; Tick 116 re-link uv Cursor env draft `1b3a12e9` + Portal Save target)  
...
| Cursor cloud environment (ICML live) | **PARTIAL** | Tick 116: draft `1b3a12e9-…` + build `bld-20260814-5f067c36-…` **SUCCEEDED** + proposed (installs **uv** 0.12.4); pointer `docs/icml_portal_save_target.json`; prior drafts not inherited — need Portal Save onto automation + secrets |
...
| ICML B vs D multi-seed GPQA | **NOT DONE** | Blocked on Portal Save of uv-capable env `1b3a12e9-…` onto automation + API keys + HF `Idavidrein/gpqa` accept; then `run_icml_live_pipeline.py --live --fetch-diamond` |
