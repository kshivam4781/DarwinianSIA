...
**Last updated:** 2026-08-20 (Section 21 ICML; Tick 165 re-link uv Cursor env draft `7af24780` + Portal Save target)  
...
| Cursor cloud environment (ICML live) | **PARTIAL** | Tick 165: draft `7af24780-…` + build `bld-20260820-9e3b0eeb-…` **SUCCEEDED** + proposed (installs **uv** 0.12.5); pointer `docs/icml_portal_save_target.json`; prior drafts not inherited — need Portal Save onto automation + secrets |
...
| ICML B vs D multi-seed GPQA | **NOT DONE** | Blocked on Portal Save of uv-capable env `7af24780-…` onto automation + API keys + HF `Idavidrein/gpqa` accept; then `run_icml_live_pipeline.py --live --fetch-diamond` |
