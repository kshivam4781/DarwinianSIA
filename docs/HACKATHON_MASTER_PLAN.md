...
**Last updated:** 2026-08-14 (Section 21 ICML; Tick 118 re-link uv Cursor env draft `75254e0e` + Portal Save target)  
...
| Cursor cloud environment (ICML live) | **PARTIAL** | Tick 118: draft `75254e0e-…` + build `bld-20260814-6aede369-…` **SUCCEEDED** + proposed (installs **uv** 0.12.4); pointer `docs/icml_portal_save_target.json`; prior drafts not inherited — need Portal Save onto automation + secrets |
...
| ICML B vs D multi-seed GPQA | **NOT DONE** | Blocked on Portal Save of uv-capable env `75254e0e-…` onto automation + API keys + HF `Idavidrein/gpqa` accept; then `run_icml_live_pipeline.py --live --fetch-diamond` |
