...
**Last updated:** 2026-08-11 (Section 21 ICML; Tick 90 re-link uv Cursor env draft `53bfbb6f` + Portal Save target)  
...
| Cursor cloud environment (ICML live) | **PARTIAL** | Tick 90: draft `53bfbb6f-…` + build `bld-20260811-8ce062cd-…` **SUCCEEDED** + proposed (installs **uv** 0.12.3); pointer `docs/icml_portal_save_target.json`; prior drafts not inherited — need Portal Save onto automation + secrets |
...
| ICML B vs D multi-seed GPQA | **NOT DONE** | Blocked on Portal Save of uv-capable env `53bfbb6f-…` onto automation + API keys + HF `Idavidrein/gpqa` accept; then `run_icml_live_pipeline.py --live --fetch-diamond` |
