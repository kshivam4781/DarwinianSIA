...
**Last updated:** 2026-08-11 (Section 21 ICML; Tick 93 re-link uv Cursor env draft `fcb0a0f4` + Portal Save target)  
...
| Cursor cloud environment (ICML live) | **PARTIAL** | Tick 93: draft `fcb0a0f4-…` + build `bld-20260811-96041347-…` **SUCCEEDED** + proposed (installs **uv** 0.12.3); pointer `docs/icml_portal_save_target.json`; prior drafts not inherited — need Portal Save onto automation + secrets |
...
| ICML B vs D multi-seed GPQA | **NOT DONE** | Blocked on Portal Save of uv-capable env `fcb0a0f4-…` onto automation + API keys + HF `Idavidrein/gpqa` accept; then `run_icml_live_pipeline.py --live --fetch-diamond` |
