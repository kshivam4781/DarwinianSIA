...
**Last updated:** 2026-08-16 (Section 21 ICML; Tick 132 re-link uv Cursor env draft `3e680d4c` + Portal Save target)  
...
| Cursor cloud environment (ICML live) | **PARTIAL** | Tick 132: draft `3e680d4c-…` + build `bld-20260816-33f67cb5-…` **SUCCEEDED** + proposed (installs **uv** 0.12.5); pointer `docs/icml_portal_save_target.json`; prior drafts not inherited — need Portal Save onto automation + secrets |
...
| ICML B vs D multi-seed GPQA | **NOT DONE** | Blocked on Portal Save of uv-capable env `3e680d4c-…` onto automation + API keys + HF `Idavidrein/gpqa` accept; then `run_icml_live_pipeline.py --live --fetch-diamond` |
