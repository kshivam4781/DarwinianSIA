...
**Last updated:** 2026-08-11 (Section 21 ICML; Tick 87 re-link uv Cursor env draft `2b9d6576` + Portal Save target)  
...
| Cursor cloud environment (ICML live) | **PARTIAL** | Tick 87: draft `2b9d6576-…` + build `bld-20260811-ee330319-…` **SUCCEEDED** + proposed (installs **uv** 0.12.3); pointer `docs/icml_portal_save_target.json`; prior drafts not inherited — need Portal Save onto automation + secrets |
...
| ICML B vs D multi-seed GPQA | **NOT DONE** | Blocked on Portal Save of uv-capable env `2b9d6576-…` onto automation + API keys + HF `Idavidrein/gpqa` accept; then `run_icml_live_pipeline.py --live --fetch-diamond` |
