...
**Last updated:** 2026-08-21 (Section 21 ICML; Tick 171 re-link uv Cursor env draft `ac65a60c` + Portal Save target)  
...
| Cursor cloud environment (ICML live) | **PARTIAL** | Tick 171: draft `ac65a60c-…` + build `bld-20260821-5c40fbd4-…` **SUCCEEDED** + proposed (installs **uv** 0.12.5); pointer `docs/icml_portal_save_target.json`; prior drafts not inherited — need Portal Save onto automation + secrets |
...
| ICML B vs D multi-seed GPQA | **NOT DONE** | Blocked on Portal Save of uv-capable env `ac65a60c-…` onto automation + API keys + HF `Idavidrein/gpqa` accept; then `run_icml_live_pipeline.py --live --fetch-diamond` |
