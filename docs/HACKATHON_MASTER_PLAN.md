...
**Last updated:** 2026-08-17 (Section 21 ICML; Tick 140 re-link uv Cursor env draft `1de8d11c` + Portal Save target)  
...
| Cursor cloud environment (ICML live) | **PARTIAL** | Tick 140: draft `1de8d11c-…` + build `bld-20260817-ae9d2731-…` **SUCCEEDED** + proposed (installs **uv** 0.12.5); pointer `docs/icml_portal_save_target.json`; prior drafts not inherited — need Portal Save onto automation + secrets |
...
| ICML B vs D multi-seed GPQA | **NOT DONE** | Blocked on Portal Save of uv-capable env `1de8d11c-…` onto automation + API keys + HF `Idavidrein/gpqa` accept; then `run_icml_live_pipeline.py --live --fetch-diamond` |
