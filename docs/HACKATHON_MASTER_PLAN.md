...
**Last updated:** 2026-08-16 (Section 21 ICML; Tick 138 re-link uv Cursor env draft `0225f827` + Portal Save target)  
...
| Cursor cloud environment (ICML live) | **PARTIAL** | Tick 138: draft `0225f827-…` + build `bld-20260816-36c10b0a-…` **SUCCEEDED** + proposed (installs **uv** 0.12.5); pointer `docs/icml_portal_save_target.json`; prior drafts not inherited — need Portal Save onto automation + secrets |
...
| ICML B vs D multi-seed GPQA | **NOT DONE** | Blocked on Portal Save of uv-capable env `0225f827-…` onto automation + API keys + HF `Idavidrein/gpqa` accept; then `run_icml_live_pipeline.py --live --fetch-diamond` |
