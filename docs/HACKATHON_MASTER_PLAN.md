...
**Last updated:** 2026-08-18 (Section 21 ICML; Tick 147 re-link uv Cursor env draft `38306c22` + Portal Save target)  
...
| Cursor cloud environment (ICML live) | **PARTIAL** | Tick 147: draft `38306c22-…` + build `bld-20260818-0a1b6261-…` **SUCCEEDED** + proposed (installs **uv** 0.12.5); pointer `docs/icml_portal_save_target.json`; prior drafts not inherited — need Portal Save onto automation + secrets |
...
| ICML B vs D multi-seed GPQA | **NOT DONE** | Blocked on Portal Save of uv-capable env `38306c22-…` onto automation + API keys + HF `Idavidrein/gpqa` accept; then `run_icml_live_pipeline.py --live --fetch-diamond` |
