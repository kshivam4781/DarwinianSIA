...
**Last updated:** 2026-08-18 (Section 21 ICML; Tick 151 re-link uv Cursor env draft `09627802` + Portal Save target)  
...
| Cursor cloud environment (ICML live) | **PARTIAL** | Tick 151: draft `09627802-…` + build `bld-20260818-7eb90e06-…` **SUCCEEDED** + proposed (installs **uv** 0.12.5); pointer `docs/icml_portal_save_target.json`; prior drafts not inherited — need Portal Save onto automation + secrets |
...
| ICML B vs D multi-seed GPQA | **NOT DONE** | Blocked on Portal Save of uv-capable env `09627802-…` onto automation + API keys + HF `Idavidrein/gpqa` accept; then `run_icml_live_pipeline.py --live --fetch-diamond` |
