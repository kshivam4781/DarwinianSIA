...
**Last updated:** 2026-08-19 (Section 21 ICML; Tick 157 re-link uv Cursor env draft `1ff2ffe2` + Portal Save target)  
...
| Cursor cloud environment (ICML live) | **PARTIAL** | Tick 157: draft `1ff2ffe2-…` + build `bld-20260819-8598414c-…` **SUCCEEDED** + proposed (installs **uv** 0.12.5); pointer `docs/icml_portal_save_target.json`; prior drafts not inherited — need Portal Save onto automation + secrets |
...
| ICML B vs D multi-seed GPQA | **NOT DONE** | Blocked on Portal Save of uv-capable env `1ff2ffe2-…` onto automation + API keys + HF `Idavidrein/gpqa` accept; then `run_icml_live_pipeline.py --live --fetch-diamond` |
