...
**Last updated:** 2026-08-11 (Section 21 ICML; Tick 89 re-link uv Cursor env draft `07261747` + Portal Save target)  
...
| Cursor cloud environment (ICML live) | **PARTIAL** | Tick 89: draft `07261747-…` + build `bld-20260811-4b0c704f-…` **SUCCEEDED** + proposed (installs **uv** 0.12.3); pointer `docs/icml_portal_save_target.json`; prior drafts not inherited — need Portal Save onto automation + secrets |
...
| ICML B vs D multi-seed GPQA | **NOT DONE** | Blocked on Portal Save of uv-capable env `07261747-…` onto automation + API keys + HF `Idavidrein/gpqa` accept; then `run_icml_live_pipeline.py --live --fetch-diamond` |
