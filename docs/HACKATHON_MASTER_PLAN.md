...
**Last updated:** 2026-08-16 (Section 21 ICML; Tick 139 re-link uv Cursor env draft `b439de3e` + Portal Save target)  
...
| Cursor cloud environment (ICML live) | **PARTIAL** | Tick 139: draft `b439de3e-…` + build `bld-20260816-a45083f0-…` **SUCCEEDED** + proposed (installs **uv** 0.12.5); pointer `docs/icml_portal_save_target.json`; prior drafts not inherited — need Portal Save onto automation + secrets |
...
| ICML B vs D multi-seed GPQA | **NOT DONE** | Blocked on Portal Save of uv-capable env `b439de3e-…` onto automation + API keys + HF `Idavidrein/gpqa` accept; then `run_icml_live_pipeline.py --live --fetch-diamond` |
