...
**Last updated:** 2026-08-16 (Section 21 ICML; Tick 131 re-link uv Cursor env draft `b386c9a9` + Portal Save target)  
...
| Cursor cloud environment (ICML live) | **PARTIAL** | Tick 131: draft `b386c9a9-…` + build `bld-20260816-7dd2b14f-…` **SUCCEEDED** + proposed (installs **uv** 0.12.5); pointer `docs/icml_portal_save_target.json`; prior drafts not inherited — need Portal Save onto automation + secrets |
...
| ICML B vs D multi-seed GPQA | **NOT DONE** | Blocked on Portal Save of uv-capable env `b386c9a9-…` onto automation + API keys + HF `Idavidrein/gpqa` accept; then `run_icml_live_pipeline.py --live --fetch-diamond` |
