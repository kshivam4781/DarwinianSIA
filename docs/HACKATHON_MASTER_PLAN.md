...
**Last updated:** 2026-08-18 (Section 21 ICML; Tick 150 re-link uv Cursor env draft `3609469a` + Portal Save target)  
...
| Cursor cloud environment (ICML live) | **PARTIAL** | Tick 150: draft `3609469a-…` + build `bld-20260818-696ac676-…` **SUCCEEDED** + proposed (installs **uv** 0.12.5); pointer `docs/icml_portal_save_target.json`; prior drafts not inherited — need Portal Save onto automation + secrets |
...
| ICML B vs D multi-seed GPQA | **NOT DONE** | Blocked on Portal Save of uv-capable env `3609469a-…` onto automation + API keys + HF `Idavidrein/gpqa` accept; then `run_icml_live_pipeline.py --live --fetch-diamond` |
