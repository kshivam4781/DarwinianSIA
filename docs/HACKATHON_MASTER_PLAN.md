...
**Last updated:** 2026-08-10 (Section 21 ICML; Tick 74 re-link uv Cursor env draft `5f5823ed` + Portal Save target)  
...
| Cursor cloud environment (ICML live) | **PARTIAL** | Tick 74: draft `5f5823ed-…` + build `bld-20260810-bd0d630d-…` **SUCCEEDED** + proposed (installs **uv** 0.12.3); pointer `docs/icml_portal_save_target.json`; prior drafts not inherited — need Portal Save onto automation + secrets |
...
| ICML B vs D multi-seed GPQA | **NOT DONE** | Blocked on Portal Save of uv-capable env `5f5823ed-…` onto automation + API keys + HF `Idavidrein/gpqa` accept; then `run_icml_live_pipeline.py --live --fetch-diamond` |
