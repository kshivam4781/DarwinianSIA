...
**Last updated:** 2026-08-10 (Section 21 ICML; Tick 83 re-link uv Cursor env draft `2bd15cd6` + Portal Save target)  
...
| Cursor cloud environment (ICML live) | **PARTIAL** | Tick 83: draft `2bd15cd6-…` + build `bld-20260810-c3fe0508-…` **SUCCEEDED** + proposed (installs **uv** 0.12.3); pointer `docs/icml_portal_save_target.json`; prior drafts not inherited — need Portal Save onto automation + secrets |
...
| ICML B vs D multi-seed GPQA | **NOT DONE** | Blocked on Portal Save of uv-capable env `2bd15cd6-…` onto automation + API keys + HF `Idavidrein/gpqa` accept; then `run_icml_live_pipeline.py --live --fetch-diamond` |
