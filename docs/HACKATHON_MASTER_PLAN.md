...
**Last updated:** 2026-08-20 (Section 21 ICML; Tick 160 re-link uv Cursor env draft `7a57b118` + Portal Save target)  
...
| Cursor cloud environment (ICML live) | **PARTIAL** | Tick 160: draft `7a57b118-…` + build `bld-20260820-17f3a0cf-…` **SUCCEEDED** + proposed (installs **uv** 0.12.5); pointer `docs/icml_portal_save_target.json`; prior drafts not inherited — need Portal Save onto automation + secrets |
...
| ICML B vs D multi-seed GPQA | **NOT DONE** | Blocked on Portal Save of uv-capable env `7a57b118-…` onto automation + API keys + HF `Idavidrein/gpqa` accept; then `run_icml_live_pipeline.py --live --fetch-diamond` |
