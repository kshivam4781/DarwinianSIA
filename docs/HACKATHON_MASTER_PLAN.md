...
**Last updated:** 2026-08-07 (Section 21 ICML; Tick 41 re-link uv Cursor env draft `b28dbfe2` + Portal Save target)  
...
| Cursor cloud environment (ICML live) | **PARTIAL** | Tick 41: draft `b28dbfe2-…` + build `bld-20260807-5b2c6af7-…` **SUCCEEDED** + proposed (installs **uv**); pointer `docs/icml_portal_save_target.json`; prior drafts not inherited — need Portal Save onto automation + secrets |
...
| ICML B vs D multi-seed GPQA | **NOT DONE** | Blocked on Portal Save of uv-capable env `b28dbfe2-…` onto automation + API keys + HF `Idavidrein/gpqa` accept; then `run_icml_live_pipeline.py --live --fetch-diamond` |
