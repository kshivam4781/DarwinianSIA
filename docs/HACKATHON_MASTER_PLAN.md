...
**Last updated:** 2026-08-12 (Section 21 ICML; Tick 103 re-link uv Cursor env draft `945cf4e0` + Portal Save target)  
...
| Cursor cloud environment (ICML live) | **PARTIAL** | Tick 103: draft `945cf4e0-…` + build `bld-20260812-ff4cb61f-…` **SUCCEEDED** + proposed (installs **uv** 0.12.3); pointer `docs/icml_portal_save_target.json`; prior drafts not inherited — need Portal Save onto automation + secrets |
...
| ICML B vs D multi-seed GPQA | **NOT DONE** | Blocked on Portal Save of uv-capable env `945cf4e0-…` onto automation + API keys + HF `Idavidrein/gpqa` accept; then `run_icml_live_pipeline.py --live --fetch-diamond` |
