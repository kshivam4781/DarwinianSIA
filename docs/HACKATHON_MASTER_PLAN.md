...
**Last updated:** 2026-08-10 (Section 21 ICML; Tick 76 re-link uv Cursor env draft `be57c785` + Portal Save target)  
...
| Cursor cloud environment (ICML live) | **PARTIAL** | Tick 76: draft `be57c785-…` + build `bld-20260810-8b16e793-…` **SUCCEEDED** + proposed (installs **uv** 0.12.3); pointer `docs/icml_portal_save_target.json`; prior drafts not inherited — need Portal Save onto automation + secrets |
...
| ICML B vs D multi-seed GPQA | **NOT DONE** | Blocked on Portal Save of uv-capable env `be57c785-…` onto automation + API keys + HF `Idavidrein/gpqa` accept; then `run_icml_live_pipeline.py --live --fetch-diamond` |
...
**Portal Save re-link (2026-08-10 Tick 76):** Cron again booted `environment: null` (Tick 75 draft not attached to automation). Re-linked uv-capable personal draft `be57c785-9491-11f1-ba66-0e7d0216e441`; build `bld-20260810-8b16e793-d70b-45a6-87ce-2d56ce23335f` **SUCCEEDED** (uv 0.12.3) + proposed; `docs/icml_portal_save_target.json` updated. User must Portal Save that draft onto automation `bf73dff3-…` and inject secrets, or every future cron will keep re-creating orphan drafts.
