...
**Last updated:** 2026-08-19 (Section 21 ICML; Tick 158 re-link uv Cursor env draft `e8dc8a19` + Portal Save target)  
...
| Cursor cloud environment (ICML live) | **PARTIAL** | Tick 158: draft `e8dc8a19-…` + build `bld-20260819-875b56ec-…` **SUCCEEDED** + proposed (installs **uv** 0.12.5); pointer `docs/icml_portal_save_target.json`; prior drafts not inherited — need Portal Save onto automation + secrets |
...
| ICML B vs D multi-seed GPQA | **NOT DONE** | Blocked on Portal Save of uv-capable env `e8dc8a19-…` onto automation + API keys + HF `Idavidrein/gpqa` accept; then `run_icml_live_pipeline.py --live --fetch-diamond` |
