...
1. No `ANTHROPIC_API_KEY` / `NEBIUS_API_KEY` in this cloud environment (verified empty; secrets re-requested 2026-08-05 Tick 23).
2. ~~Bundled GPQA `data/public` missing~~ **layout unblocked** Tick 21 via `scripts/prepare_gpqa_smoke_data.py` + CLI dry-run `run_1800`. Live G2/G3 still need **real** GPQA diamond JSON (HF `Idavidrein/gpqa` is gated) + keys.
