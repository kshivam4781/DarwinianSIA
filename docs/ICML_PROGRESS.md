...
- `main_has_icml_tip`: **false** (origin/main still lacks `scripts/icml_cron_entry.sh`)
- Tip PR title+body still stale: **Tick 336** on GitHub
- Boot branch was greenfield `cursor/icml-epistemic-results-6152`; recovered tip `f49c` (needed temporary boot-file move — Tick 391 hole)
- Secrets re-filed via `request-environment-setup-actions` (Portal Save skipped)

### Largest gap diagnosed
Live PRIMARY still blocked on **secrets**. Tick 391 filtered gitignore-lag durables only when `scripts/icml_env_checks.py` was already on disk; chicken-egg `git show tip:…/icml_boot_recover.sh | bash -s -- --apply` still refused on `?? docs/icml_cloud_boot_branch.txt` because the Python filter never ran. Highest leverage without paid spend: **inline IGNORE in boot_recover/cron when tip module absent**.

### What this tick did (ONE step)
**Chicken-egg tip-apply without tip module (no API spend; tip PR #337 updated in place):**
1. Recovered tip ← Tick 391 (`f49c`); confirmed secrets absent; boot `6152` vs tip `f49c`; re-filed NEBIUS+HF secrets request
2. `icml_boot_recover.sh` + `icml_cron_entry.sh`: when `scripts/icml_env_checks.py` is missing, inline the same IGNORE set as `TIP_APPLY_GITIGNORE_LAG_RELPATHS` (boot/call/stash); evidence still blocks (Tick 390)
3. Tests: `test_boot_recover_chicken_egg_filters_boot_without_env_checks` + source lock Tick 392 — STATUS remains IN_PROGRESS; secrets still required for live PRIMARY

### Metrics delta
| Metric | Before (Tick 391) | After (Tick 392) |
|--------|-------------------|------------------|
| Offline D final / gens30 / cost30 / H5 / H2 | 5/5 / 4/5 / 4/5 / 5/5 / 4/5 | unchanged |
| Greenfield `git show tip:boot_recover \| bash --apply` + boot file | refuses `--apply` (no tip module) | **applies (inline IGNORE)** |
| Dirty prior_live evidence vs tip `--apply` | blocks `--apply` | unchanged |
| Live PRIMARY / G2 | Blocked on NEBIUS + HF/CSV | Still blocked |
| `ICML_READY` | IN_PROGRESS | IN_PROGRESS |
...
---
## 2026-09-09T04:00Z — Tick 391 (automation cron)
...
- `main_has_icml_tip`: **false** (origin/main still lacks `scripts/icml_cron_entry.sh`)
- Tip PR title+body still stale: **Tick 336** on GitHub
- Boot branch was greenfield `cursor/icml-epistemic-results-dd06`; recovered tip `f49c`
- Secrets re-filed via `request-environment-setup-actions` (Portal Save skipped)

### Largest gap diagnosed
Live PRIMARY still blocked on **secrets**. Tick 390 made dirty prior_live evidence block tip `--apply`, but chicken-egg greenfield boots (no tip `.gitignore`) persist `docs/icml_cloud_boot_branch.txt` *before* tip recover — porcelain then shows the boot file as dirty and **refuses `--apply`**, so cron never lands tip files. Highest leverage without paid spend: **tip-apply ignores gitignore-lag durables**.

### What this tick did (ONE step)
**Tip-apply ignores gitignore-lag durables (no API spend; tip PR #337 updated in place):**
1. Recovered tip ← Tick 390 (`f49c`); confirmed secrets absent; boot `dd06` vs tip `f49c`; re-filed NEBIUS+HF secrets request
2. `TIP_APPLY_GITIGNORE_LAG_RELPATHS` (boot file + open_git_pr call JSON + prior_live stash) filtered in `tip_apply_blocking_dirty_paths` / `discard_ephemeral_icml_dirt` / `icml_recover_tip.py --apply` (shared helper); fixed Tick 390 undefined `evidence_norm` in post-discard remaining check; evidence still blocks (Tick 390)
3. Tests: `test_tip_apply_ignores_gitignore_lag_boot_and_call` / `test_discard_ephemeral_gitignore_lag_boot_ok` / recover_tip source lock — STATUS remains IN_PROGRESS; secrets still required for live PRIMARY

### Metrics delta
| Metric | Before (Tick 390) | After (Tick 391) |
|--------|-------------------|------------------|
| Offline D final / gens30 / cost30 / H5 / H2 | 5/5 / 4/5 / 4/5 / 5/5 / 4/5 | unchanged |
| Greenfield boot file vs tip `--apply` | blocks `--apply` (gitignore lag) | **ignored (with call/stash)** |
| Dirty prior_live evidence vs tip `--apply` | blocks `--apply` | unchanged (ledger parity) |
| Live PRIMARY / G2 | Blocked on NEBIUS + HF/CSV | Still blocked |
| `ICML_READY` | IN_PROGRESS | IN_PROGRESS |
...
---
## 2026-09-09T02:15Z — Tick 390 (automation cron)
...
- `main_has_icml_tip`: **false** (origin/main still lacks `scripts/icml_cron_entry.sh`)
- Tip PR title+body still stale: **Tick 336** on GitHub
- Boot branch was greenfield `cursor/icml-epistemic-results-8b3a`; recovered tip `f49c`
- Secrets re-filed via `request-environment-setup-actions` (Portal Save skipped)

### Largest gap diagnosed
Live PRIMARY still blocked on **secrets**. Tick 389 committed `docs/icml_prior_live_evidence.json` for cross-VM reinject, but excluded that file from tip `--apply` dirty filters so hard-reset could wipe uncommitted gates and rely on same-VM stash — **not** true budget-ledger parity (dirty ledger already blocks `--apply`). Highest leverage without paid spend: **tip-apply blocks dirty prior_live evidence**.

### What this tick did (ONE step)
**Tip-apply blocks dirty prior_live evidence (no API spend; tip PR #337 updated in place):**
1. Recovered tip ← Tick 389 (`f49c`); confirmed secrets absent; boot `8b3a` vs tip `f49c`; re-filed NEBIUS+HF secrets request
2. `tip_apply_blocking_dirty_paths` / `discard_ephemeral_icml_dirt` / `icml_recover_tip.py --apply` now ignore **stash only** (dirty evidence blocks `--apply` like `icml_budget_spent.json`); refreshed `suggested_open_git_pr_body` Tick text
3. Tests: `test_tip_apply_blocks_dirty_prior_live_evidence` / `test_discard_ephemeral_blocks_dirty_prior_live_evidence` / `test_recover_tip_apply_source_does_not_filter_evidence` (+ Tick 389 regression) — STATUS remains IN_PROGRESS; secrets still required for live PRIMARY

### Metrics delta
| Metric | Before (Tick 389) | After (Tick 390) |
|--------|-------------------|------------------|
| Offline D final / gens30 / cost30 / H5 / H2 | 5/5 / 4/5 / 4/5 / 5/5 / 4/5 | unchanged |
| Dirty prior_live evidence vs tip `--apply` | filtered (wipe + stash reinject) | **blocks `--apply` (ledger parity)** |
| Live PRIMARY / G2 | Blocked on NEBIUS + HF/CSV | Still blocked |
| `ICML_READY` | IN_PROGRESS | IN_PROGRESS |

### Next recommended step
Human: (1) add `NEBIUS_API_KEY` (+ `HF_TOKEN` or drop `gpqa_diamond.csv`) — **PRIMARY path**; (2) optional: refresh tip PR #337 title/body via `tip_pr_title_edit_commands` and/or merge #337/#338. Then: `bash scripts/icml_cron_entry.sh` → G2→G3→G4 + paper pack → STATUS READY when criteria pass. After live, **commit** `docs/icml_prior_live_evidence.json` with the tip before tip `--apply` (Tick 390 enforces).
...
- `main_has_icml_tip`: **false** (origin/main still lacks `scripts/icml_cron_entry.sh`)
- Tip PR title+body still stale: **Tick 336** on GitHub
- Boot branch was greenfield `cursor/icml-epistemic-results-3c61`; recovered tip `f49c`
- Secrets re-filed via `request-environment-setup-actions` (Portal Save skipped)

### Largest gap diagnosed
Live PRIMARY still blocked on **secrets**. Tick 387 wired prior_live stash+reinject into cron + `icml_boot_recover.sh`, but **`icml_recover_tip.py --apply`** (agent chicken-egg / mid-tick recover) still refused dirty trees without stashing and never reinjected after `git reset --hard` — so paid sidecar trust was still wipeable on that path. Highest leverage without paid spend: **recover_tip prior_live stash parity**.

### What this tick did (ONE step)
**`icml_recover_tip.py --apply` prior_live stash (no API spend; tip PR #337 updated in place):**
1. Recovered tip ← Tick 387 (`f49c`); confirmed secrets absent; boot `3c61` vs tip `f49c`; re-filed NEBIUS+HF secrets request
2. `apply_tip` now calls `discard_ephemeral_icml_dirt` (stashes prior_live_*) before dirty check, filters gitignored stash from dirty porcelain, and `reinject_prior_live_stash` after hard-reset (Tick 387 cron/boot_recover parity); refreshed `suggested_open_git_pr_body` Tick text
3. Tests: `test_recover_tip_apply_wires_prior_live_stash` / `test_recover_tip_apply_source_mentions_prior_live` (+ Tick 387 stash regression) — **3/3**; STATUS remains IN_PROGRESS; secrets still required for live PRIMARY

### Metrics delta
| Metric | Before (Tick 387) | After (Tick 388) |
|--------|-------------------|------------------|
| Offline D final / gens30 / cost30 / H5 / H2 | 5/5 / 4/5 / 4/5 / 5/5 / 4/5 | unchanged |
| Tip recover paths with prior_live stash | cron + boot_recover only | **+ `icml_recover_tip.py --apply`** |
| Live PRIMARY / G2 | Blocked on NEBIUS + HF/CSV | Still blocked |
| `ICML_READY` | IN_PROGRESS | IN_PROGRESS |
...
---
## 2026-09-08T20:06Z — Tick 387 (automation cron)
...
- `main_has_icml_tip`: **false** (origin/main still lacks `scripts/icml_cron_entry.sh`)
- Tip PR title+body still stale: **Tick 336** on GitHub
- Boot branch was greenfield `cursor/icml-epistemic-results-61e2`; recovered tip `f49c`
- Secrets re-filed via `request-environment-setup-actions` (Portal Save skipped)

### Largest gap diagnosed
Live PRIMARY still blocked on **secrets**. After Tick 386 completed the G2/G3/G4 `prior_live_*` write-path triad, tip recover still wiped that evidence: `discard_ephemeral_icml_dirt` → `git restore` of gate JSON, then `git reset --hard` on tip `--apply`, so paid sidecar trust never survived mid-stack tip recover. Highest leverage without paid spend: **durable prior_live stash + reinject after tip apply**.

### What this tick did (ONE step)
**Discard/tip-apply prior_live stash (no API spend; tip PR #337 updated in place):**
1. Recovered tip ← Tick 386 (`f49c`); confirmed secrets absent; boot `61e2` vs tip `f49c`; re-filed NEBIUS+HF secrets request
2. Added `persist_prior_live_stash_from_working_tree` / `reinject_prior_live_stash` + gitignored `docs/icml_prior_live_stash.json`; `discard_ephemeral_icml_dirt` stashes before restore; cron + `icml_boot_recover.sh` reinject after tip `--apply`
3. Tests: `test_discard_ephemeral_preserves_prior_live_via_stash` / `test_stash_prior_live_from_live_executed_gate3` / `test_stash_prior_live_from_live_gate2_post` (+ regression clears_reports_only) — **4/4**; STATUS remains IN_PROGRESS; secrets still required for live PRIMARY

### Metrics delta
| Metric | Before (Tick 386) | After (Tick 387) |
|--------|-------------------|------------------|
| Offline D final / gens30 / cost30 / H5 / H2 | 5/5 / 4/5 / 4/5 / 5/5 / 4/5 | unchanged |
| Tip recover discard + hard-reset | wiped prior_live_* from dirty gate JSON | **stash → reinject (Tick 387)** |
| G2/G3/G4 prior_live write-path | triad complete (Ticks 384–386) | + discard/tip-apply survival |
| Live PRIMARY / G2 | Blocked on NEBIUS + HF/CSV | Still blocked |
| `ICML_READY` | IN_PROGRESS | IN_PROGRESS |
...
- `main_has_icml_tip`: **false** (origin/main still lacks `scripts/icml_cron_entry.sh`)
- Tip PR title+body still stale: **Tick 336** on GitHub
- Boot branch was greenfield `cursor/icml-epistemic-results-c121`; recovered tip `f49c`

### Largest gap diagnosed
Live PRIMARY still blocked on **secrets**. After Tick 384's gate2 `prior_live_post` preserve, cron `--preflight-only` still wiped live gate3 `comparison`/H2/H5 (`mode=preflight`, `executed=False`), so pipeline `load_g3_metrics_for_g4` / direct G3 ledger-skip could not trust paid G3 evidence on cross-VM resume. Highest leverage without paid spend: **pipeline G3→G4 prior_live_metrics preserve**.

### What this tick did (ONE step)
**Pipeline G3→G4 prior_live_metrics preserve (no API spend; tip PR #337 updated in place):**
1. Recovered tip ← Tick 384 (`f49c`); confirmed secrets absent; boot `c121` vs tip `f49c`
2. Added `_live_metrics_from_gate3_sidecar` + `prior_live_metrics` preserve in `write_gate3_report`; wired into `refresh_g3_metrics_on_ledger_skip` + pipeline `load_g3_metrics_for_g4`
3. Tests: `test_write_gate3_preflight_preserves_prior_live_metrics` / `test_refresh_g3_metrics_on_ledger_skip_trusts_prior_live_metrics` / `test_load_g3_metrics_for_g4_trusts_prior_live_metrics` (+ regression trusts_sidecar / refuses_preflight) — **6/6**; STATUS remains IN_PROGRESS; secrets still required for live PRIMARY

### Metrics delta
| Metric | Before (Tick 384) | After (Tick 385) |
|--------|-------------------|------------------|
| Offline D final / gens30 / cost30 / H5 / H2 | 5/5 / 4/5 / 4/5 / 5/5 / 4/5 | unchanged |
| Gate3 preflight after live | wiped comparison (mode=preflight) | **preserves `prior_live_metrics`** |
| Pipeline G3→G4 / ledger-skip after wipe | refuse (no live mode) | **trust prior_live_metrics (Tick 385)** |
| Live PRIMARY / G2 | Blocked on NEBIUS + HF/CSV | Still blocked |
| `ICML_READY` | IN_PROGRESS | IN_PROGRESS |
...
---
## 2026-09-08T14:15Z — Tick 384 (automation cron)
...
- `main_has_icml_tip`: **false** (origin/main still lacks `scripts/icml_cron_entry.sh`)
- Tip PR title+body still stale: **Tick 336** on GitHub
- Boot branch was greenfield `cursor/icml-epistemic-results-7a5d`; recovered tip `f49c`

### Largest gap diagnosed
Live PRIMARY still blocked on **secrets**. After Tick 383's direct G2 ledger-skip post refresh, the **pipeline** resume-skip still advanced to paid G3 when the ledger marked G2 complete **without** re-checking gate2 post — and cron `--preflight-only` wiped live `post=` from the sidecar. Highest leverage without paid spend: **pipeline G2→G3 post-gate + prior_live_post preserve**.

### What this tick did (ONE step)
**Pipeline G2→G3 post-gate + prior_live_post preserve (no API spend; tip PR #337 updated in place):**
1. Recovered tip ← Tick 383 (`f49c`); confirmed secrets absent; boot `7a5d` vs tip `f49c`
2. Added `load_g2_post_for_g3` (local validate or live/`prior_live_post` sidecar); wired before G3 in `run_live_stack`; direct G2 ledger-skip returns exit **4** without proven post; `write_gate2_report` preserves `prior_live_post` across preflight
3. Tests: `test_write_gate2_preflight_preserves_prior_live_post` / `test_g2_live_ledger_skip_refuses_without_post` / `test_load_g2_post_for_g3_*` / `test_live_stack_refuses_g3_without_g2_post` (**10/10** focused); STATUS remains IN_PROGRESS; secrets still required for live PRIMARY

### Metrics delta
| Metric | Before (Tick 383) | After (Tick 384) |
|--------|-------------------|------------------|
| Offline D final / gens30 / cost30 / H5 / H2 | 5/5 / 4/5 / 4/5 / 5/5 / 4/5 | unchanged |
| Pipeline G2 resume → G3 | ledger-only skip without post check | **require post / prior_live_post (Tick 384)** |
| Direct G2 ledger-skip without post | exit 0 (false-green) | **exit 4** |
| Preflight gate2 rewrite | wiped live `post=` | **preserves `prior_live_post`** |
| Live PRIMARY / G2 | Blocked on NEBIUS + HF/CSV | Still blocked |
| `ICML_READY` | IN_PROGRESS | IN_PROGRESS |
...
---
## 2026-09-08T12:15Z — Tick 383 (automation cron)
...
- `main_has_icml_tip`: **false** (origin/main still lacks `scripts/icml_cron_entry.sh`)
- Tip PR title+body still stale: **Tick 336** on GitHub
- Boot branch was greenfield `cursor/icml-epistemic-results-fe4c`; recovered tip `f49c`

### Largest gap diagnosed
Live PRIMARY still blocked on **secrets**. After Tick 382's direct G3 ledger-skip metrics refresh (and Tick 381 G4 paper-pack), direct G2 `--live` ledger-skip still wrote gate2 **without `post=`**, wiping live-executed nonzero-fitness / belief_store evidence from the sidecar. Highest leverage without paid spend: **direct G2 ledger-skip post-validation refresh** (complete the G2/G3/G4 ledger-skip refresh triad).

### What this tick did (ONE step)
**Direct G2 ledger-skip post-validation refresh (no API spend; tip PR #337 updated in place):**
1. Recovered tip ← Tick 382 (`f49c`); confirmed secrets absent; boot `fe4c` vs tip `f49c`
2. Added `refresh_g2_post_on_ledger_skip` (local `validate_g2_artifacts` or trust live-executed gate2 sidecar; never invent post from preflight); wired into direct G2 `--live` ledger-skip path
3. Tests: `test_refresh_g2_post_on_ledger_skip_local_dirs` / `test_refresh_g2_post_on_ledger_skip_trusts_sidecar` / `test_refresh_g2_post_on_ledger_skip_refuses_preflight_sidecar` / `test_g2_live_ledger_skip_refreshes_post` (5/5 with Tick 380 skip); STATUS remains IN_PROGRESS; secrets still required for live PRIMARY

### Metrics delta
| Metric | Before (Tick 382) | After (Tick 383) |
|--------|-------------------|------------------|
| Offline D final / gens30 / cost30 / H5 / H2 | 5/5 / 4/5 / 4/5 / 5/5 / 4/5 | unchanged |
| Direct G2 `--live` when ledger stage done | skip + write gate2 without post (clobber risk) | **skip + post refresh / sidecar trust (Tick 383)** |
| Live PRIMARY / G2 | Blocked on NEBIUS + HF/CSV | Still blocked |
| `ICML_READY` | IN_PROGRESS | IN_PROGRESS |
...
---
## 2026-09-08T10:15Z — Tick 382 (automation cron)
...
- `main_has_icml_tip`: **false** (origin/main still lacks `scripts/icml_cron_entry.sh`)
- Tip PR title+body still stale: **Tick 336** on GitHub
- Boot branch was greenfield `cursor/icml-epistemic-results-7519`; recovered tip `f49c`

### Largest gap diagnosed
Live PRIMARY still blocked on **secrets**. After Tick 381's direct G4 ledger-skip paper-pack refresh, direct G3 `--live` ledger-skip still wrote `executed=False` with null comparison — clobbering any live-executed gate3 sidecar that pipeline Tick 373 needs for G4 advance when local `runs/` are absent. Highest leverage without paid spend: **direct G3 ledger-skip pilot-metrics refresh**.

### What this tick did (ONE step)
**Direct G3 ledger-skip pilot-metrics refresh (no API spend; tip PR #337 updated in place):**
1. Recovered tip ← Tick 381 (`f49c`); confirmed secrets absent; boot `7519` vs tip `f49c`
2. Added `refresh_g3_metrics_on_ledger_skip` (local B/D re-score via `score_pilot`, else trust live-executed gate3 sidecar; never invent metrics from preflight sidecar); wired into direct G3 `--live` ledger-skip path
3. Tests: `test_refresh_g3_metrics_on_ledger_skip_local_dirs` / `test_refresh_g3_metrics_on_ledger_skip_trusts_sidecar` / `test_refresh_g3_metrics_on_ledger_skip_refuses_preflight_sidecar` / `test_g3_live_ledger_skip_refreshes_metrics` (4/4); STATUS remains IN_PROGRESS; secrets still required for live PRIMARY

### Metrics delta
| Metric | Before (Tick 381) | After (Tick 382) |
|--------|-------------------|------------------|
| Offline D final / gens30 / cost30 / H5 / H2 | 5/5 / 4/5 / 4/5 / 5/5 / 4/5 | unchanged |
| Direct G3 `--live` when ledger stage done | skip + write empty `executed=False` (sidecar clobber risk) | **skip + metrics refresh / sidecar trust (Tick 382)** |
| Live PRIMARY / G2 | Blocked on NEBIUS + HF/CSV | Still blocked |
| `ICML_READY` | IN_PROGRESS | IN_PROGRESS |
...
---
## 2026-09-08T08:15Z — Tick 381 (automation cron)
...
- `main_has_icml_tip`: **false** (origin/main still lacks `scripts/icml_cron_entry.sh`)
- Tip PR title+body still stale: **Tick 336** on GitHub
- Boot branch was greenfield `cursor/icml-epistemic-results-0961`; recovered tip `f49c`

### Largest gap diagnosed
Live PRIMARY still blocked on **secrets**. After Tick 380's direct gate ledger-stage skip, direct G4 `--live` early-returned on ledger-complete **without** paper-pack refresh (pipeline Tick 374 already refreshes on resume). Cross-VM / same-VM ledger skip could leave `ICML_READY` stuck IN_PROGRESS after paid G4 evidence existed. Highest leverage without paid spend: **direct G4 ledger-skip paper-pack refresh**.

### What this tick did (ONE step)
**Direct G4 ledger-skip paper-pack refresh (no API spend; tip PR #337 updated in place):**
1. Recovered tip ← Tick 380 (`f49c`); confirmed secrets absent; boot `0961` vs tip `f49c`
2. Added `refresh_paper_pack_on_ledger_skip` (local B/D re-score via `apply_paper_pack`, else trust live-executed gate4 sidecar; never promote READY from preflight sidecar); wired into direct G4 `--live` ledger-skip path
3. Tests: `test_refresh_paper_pack_on_ledger_skip_local_dirs` / `test_refresh_paper_pack_on_ledger_skip_trusts_sidecar` / `test_refresh_paper_pack_on_ledger_skip_refuses_preflight_sidecar` / `test_g4_live_ledger_skip_refreshes_paper_pack` (4/4); STATUS remains IN_PROGRESS; secrets still required for live PRIMARY

### Metrics delta
| Metric | Before (Tick 380) | After (Tick 381) |
|--------|-------------------|------------------|
| Offline D final / gens30 / cost30 / H5 / H2 | 5/5 / 4/5 / 4/5 / 5/5 / 4/5 | unchanged |
| Direct G4 `--live` when ledger stage done | skip paid re-run (no paper pack) | **skip + paper-pack refresh / sidecar trust (Tick 381)** |
| Live PRIMARY / G2 | Blocked on NEBIUS + HF/CSV | Still blocked |
| `ICML_READY` | IN_PROGRESS | IN_PROGRESS |
...
- `main_has_icml_tip`: **false** (origin/main still lacks `scripts/icml_cron_entry.sh`)
- Tip PR title+body still stale: **Tick 336** on GitHub
- Boot branch was greenfield `cursor/icml-epistemic-results-fe7c`; recovered tip `f49c`

### Largest gap diagnosed
Live PRIMARY still blocked on **secrets**. After Tick 377's direct G3/G4 budget hydrate, **direct** `run_g2_smoke.py --live` still read `SIA_BUDGET_SPENT_USD` from env only (often 0). With prior G3/G4 spend already in the ledger, a standalone G2 launch could green-light over the ~$20 ceiling (G3/G4-only Tick 377 bypass). Highest leverage without paid spend: **direct G2 budget hydrate**.

### What this tick did (ONE step)
**Direct G2 budget hydrate (no API spend; tip PR #337 updated in place):**
1. Recovered tip ← Tick 377 (`f49c`); confirmed secrets absent; boot `fe7c` vs tip `f49c`
2. Extended `hydrate_direct_gate_budget_spent` with `run_estimate_usd` (single-run fallback); wired into G2 `run_preflight` before budget check
3. Tests: `test_hydrate_direct_gate_run_estimate_usd` / `test_g2_preflight_hydrates_budget_from_ledger` / `test_g2_preflight_hydrates_budget_from_unbilled_local`; STATUS remains IN_PROGRESS; secrets re-requested

### Metrics delta
| Metric | Before (Tick 377) | After (Tick 378) |
|--------|-------------------|------------------|
| Offline D final / gens30 / cost30 / H5 / H2 | 5/5 / 4/5 / 4/5 / 5/5 / 4/5 | unchanged |
| Direct G3/G4 `--live` budget | ledger + unbilled local completes | unchanged |
| Direct G2 `--live` budget | env-only (often $0) | **ledger + unbilled local completes** (`run_estimate_usd`) |
| Live PRIMARY / G2 | Blocked on NEBIUS + HF/CSV | Still blocked |
| `ICML_READY` | IN_PROGRESS | IN_PROGRESS |
...
- `main_has_icml_tip`: **false** (origin/main still lacks `scripts/icml_cron_entry.sh`)
- Tip PR title+body still stale: **Tick 336** on GitHub
- Boot branch was greenfield `cursor/icml-epistemic-results-4496`; recovered tip `f49c`

### Largest gap diagnosed
Live PRIMARY still blocked on **secrets**. After Tick 371's nonzero-fitness G2 post-run gate, a **0%-fitness G2 still writes `results.json`**, so `darwinian_run_complete` / Tick 284 resume would treat G2 as done on the next cron and auto-burn ~$19 on G3+G4. Highest leverage without paid spend: **G2 resume post-run re-validation**.

### What this tick did (ONE step)
**G2 resume post-run re-validation (no API spend; tip PR #337 updated in place):**
1. Recovered tip ← Tick 371 (`f49c`); confirmed secrets absent; boot `4496` vs tip `f49c`
2. `g2_resume_gates_ok` + `sync_spent_from_completed_stages` re-runs `validate_g2_artifacts` on local G2; refuses resume-complete / live advance when gates fail (pick new `--g2-run-id`)
3. Tests: `test_g2_resume_refuses_zero_fitness_local_artifacts`; STATUS remains IN_PROGRESS; secrets re-requested

### Metrics delta
| Metric | Before (Tick 371) | After (Tick 372) |
|--------|-------------------|------------------|
| Offline D final / gens30 / cost30 / H5 / H2 | 5/5 / 4/5 / 4/5 / 5/5 / 4/5 | unchanged |
| G2 post-run PASS | best fitness > 0 required | unchanged |
| G2 **resume** after 0%-fitness exit 4 | would skip G2 (`results.json` present) | **re-validate; refuse G3/G4** |
| Live PRIMARY / G2 | Blocked on NEBIUS + HF/CSV | Still blocked |
| `ICML_READY` | IN_PROGRESS | IN_PROGRESS |
...
- `main_has_icml_tip`: **false** (origin/main still lacks `scripts/icml_cron_entry.sh`)
- Tip PR title+body still stale: **Tick 336** on GitHub
- Boot branch was greenfield `cursor/icml-epistemic-results-b5a1`; recovered tip `f49c`

### Largest gap diagnosed
Live PRIMARY still blocked on **secrets**. After Tick 369 surfaced H2 + `mean_final_gap` in the live pipeline **report**, `g3_pilot_promising` could still auto-spend ~$14 on 5-seed G4 when the G3 pilot had **only** H5 ρ>0.3 (no PRIMARY-shaped D win) — burning the ceiling on VALIDITY without end-goal criterion 1. Highest leverage without paid spend: **tighten G3→G4 gate to PRIMARY-only**.

### What this tick did (ONE step)
**G3→G4 PRIMARY-only promising gate (no API spend; tip PR #337 updated in place):**
1. Recovered tip ← Tick 369 (`f49c`); confirmed secrets absent; boot `b5a1` vs tip `f49c`
2. `g3_pilot_promising` requires gens/cost/final wins or mean_final_gap>1pp; H5 alone returns False; skip reason updated; tip body template refreshed
3. Tests: `test_g3_pilot_promising_on_d_win`; STATUS remains IN_PROGRESS; secrets re-requested

### Metrics delta
| Metric | Before (Tick 369) | After (Tick 370) |
|--------|-------------------|------------------|
| Offline D final / gens30 / cost30 / H5 / H2 | 5/5 / 4/5 / 4/5 / 5/5 / 4/5 | unchanged |
| Live pipeline G3→G4 report | mean_final_gap + H2 preferred | unchanged |
| Live pipeline G3→G4 **decision** | PRIMARY **or H5 alone** | **PRIMARY-shaped only** (H5 report-only; `--force-g4`) |
| Live PRIMARY / G2 | Blocked on NEBIUS + HF/CSV | Still blocked |
| `ICML_READY` | IN_PROGRESS | IN_PROGRESS |
...
- `main_has_icml_tip`: **false** (origin/main still lacks `scripts/icml_cron_entry.sh`)
- Tip PR title+body still stale: **Tick 336** on GitHub
- Boot branch was greenfield `cursor/icml-epistemic-results-1729`; recovered tip `f49c`

### Largest gap diagnosed
Live PRIMARY still blocked on **secrets**. After Tick 367 wired G4 paper/gate4 H2 preferred-pass aggregate, **live G3** still scored only compare+H5 — no H2 preferred-share / mean_final_gap in gate3 live metrics — so G3→G4 operators would miss MECHANISM + PRIMARY criterion (c) before spending on 5-seed G4. Highest leverage without paid spend: **G3 live H2 + mean_final_gap surfacing (G4 Tick 360/367 parity)**.

### What this tick did (ONE step)
**Live G3 H2 preferred-share + mean_final_gap (no API spend; tip PR #337 updated in place):**
1. Recovered tip ← Tick 367 (`f49c`); confirmed secrets absent; boot `1729` vs tip `f49c`
2. `score_pilot` returns H2 via `score_live_h2`; `write_gate3_report` surfaces mean_final_gap / primary_final_pass / d_wins_h2 / h2_preferred_pass + per-run preferred_share; G4 `apply_paper_pack` unpacks 3-tuple (no double H2)
3. Tests: gate3 report H2+gap + score_pilot triple + apply_paper_pack; STATUS remains IN_PROGRESS; secrets re-requested

### Metrics delta
| Metric | Before (Tick 367) | After (Tick 368) |
|--------|-------------------|------------------|
| Offline D final / gens30 / cost30 / H5 / H2 | 5/5 / 4/5 / 4/5 / 5/5 / 4/5 | unchanged |
| Live G4 H2 report | `d_wins_h2` + `h2_preferred_pass` | unchanged |
| Live G3 metrics | gens/cost/final + H5 only | **+ mean_final_gap + H2 preferred** |
| Live PRIMARY / G2 | Blocked on NEBIUS + HF/CSV | Still blocked |
| `ICML_READY` | IN_PROGRESS | IN_PROGRESS |
...
- `main_has_icml_tip`: **false** (origin/main still lacks `scripts/icml_cron_entry.sh`)
- Tip PR title+body still stale: **Tick 336** on GitHub
- Boot branch was greenfield `cursor/icml-epistemic-results-4ece`; recovered tip `f49c`

### Largest gap diagnosed
Live PRIMARY still blocked on **secrets**. After Tick 365 surfaced honest preferred shares (seed 22 ≈0.29), compare/paper still lacked a MECHANISM seed-win aggregate — gate3/Table 2 could not report preferred-pass **4/5** vs pool-era all-pass. Highest leverage without paid spend: **add `d_wins_h2` / `h2_preferred_pass` + brief `D_h2_pass`**.

### What this tick did (ONE step)
**Offline H2 preferred-pass aggregate (no API spend; tip PR #337 updated in place):**
1. Recovered tip ← Tick 365 (`f49c`); confirmed secrets absent; boot `4ece` vs tip `f49c`
2. `h2_preferred_seed_pass` + `compare_b_vs_d` emits `d_wins_h2` / `h2_preferred_pass`; offline brief `_brief_h2_fields` adds `D_h2_pass`
3. Patched `offline_bvd_summary.json` + gate3 offline block + paper Table 2 — preferred **4/5** (seed 22 fail); PRIMARY/H5 unchanged
4. Tests: brief H2 pass + compare aggregate; STATUS remains IN_PROGRESS; secrets re-requested

### Metrics delta
| Metric | Before (Tick 365) | After (Tick 366) |
|--------|-------------------|------------------|
| Offline D final / gens30 / cost30 / H5 | 5/5 / 4/5 / 4/5 / 5/5 | unchanged |
| Offline H2 preferred ≥0.5 | shares only (no aggregate) | **`d_wins_h2=4/5`**, `h2_preferred_pass=true` |
| Seed 22 preferred | ~0.29 (honest) | still fail; now counted |
| Live PRIMARY / G2 | Blocked on NEBIUS + HF/CSV | Still blocked |
| `ICML_READY` | IN_PROGRESS | IN_PROGRESS |
